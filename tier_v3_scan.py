...
# 🥇 **إقرارٌ مؤرَّخ 2026-08-26 («اعتمد v1»):** `V0` صارت **تاريخًا لا
#    الإنتاجَ الحاليّ** — الإنتاجُ اعتمد `V1` بأمر المالك، والأداةُ تبقى
#    مجمَّدةً على أذرعها المنشورة (سابقةُ `CAP15`): `V0` = `S2` **كما
#    نُشرت** كي تبقى بوّابةُ التكامل `G1` قابلةً لإعادة الإنتاج بت-بت.
ARMS = (
    ("V0", f"الأساس = `S2` — الإنتاجُ السابق قبل اعتماد v1 (أخضر {SMIN} فأكثر)"),
