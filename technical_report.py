...
def scan_nasdaq_earnings():
    ...
    try:
        ...
        return None
    # P1-6 (تدقيق Codex 2026-07-14 · عرض/صدق فقط — أداة مستقلة، لا تمسّ الفرز):
    # `get_universe` **يبتلع** فشل الشبكة (403/خنق) لكل رابط ويرجع قائمة **فارغة** بلا
    # استثناء — ففحص الاستثناء أعلاه لا يلتقطه، ويصل «لا توجد أسهم تطابق الشروط» =
    # اطمئنان كاذب بينما المسح **لم يعمل أصلًا**. كون فارغ = تعذُّر مسح، لا سوق فاضٍ.
    if not universe:
        bot.log("تعذّر جلب الكون: قائمة ناسداك رجعت فارغة (فشل شبكة/حظر غالبًا).")
        return None
    ...
    try:
        ...
        return None
    # P1-6 (مراجعة Codex على main-safety): `download_history` **يبتلع** فشل كل دفعة ويرجع
    # ما نجا فقط بلا استثناء — فحارس الاستثناء لا يلتقط الانقطاع. **تغطية جزئية شديدة**
    # (رمز واحد من آلاف) كانت تمرّ فيُعرض «لا توجد أسهم» بينما السوق كله لم يُمسح. نعيد
    # استخدام سياسة الصحّة نفسها (`DATA_HEALTH_MIN_PCT`): تغطية أقل من الحدّ = تعذُّر مسح.
    _cov = (len(data) / len(universe) * 100.0) if universe else 0.0
    if _cov < bot.CONFIG["DATA_HEALTH_MIN_PCT"]:
        bot.log(f"تعذّر التحميل: تغطية {_cov:.0f}% فقط "
                f"({len(data)}/{len(universe)}، الحدّ {bot.CONFIG['DATA_HEALTH_MIN_PCT']}%) — "
                "تحميل ناقص لا يُعرَض كـ«صفر مرشّح».")
        return None
...
def render_scan(results):
    ...
    # P1-6 (تدقيق Codex 2026-07-14): `None` = **تعذّر المسح** · `[]` = مُسِح ولا مرشّح.
    # كانا يُدمجان في رسالة واحدة مطمئنة («لا توجد أسهم») فيختفي عطل الشبكة بصمت —
    # الفشل يجب أن يُقال، لا أن يُقرأ كسوقٍ فاضٍ. عرض/صدق فقط (أداة مستقلة).
    if results is None:
        return ("🗓️ <b>مرشحو ناسداك (قوي فنياً + إعلان أرباح قريب)</b>\n"
                f"التاريخ: {today}\n\n"
                "⚠️ <b>تعذّر المسح اليوم</b> — لم نصل لقائمة ناسداك أو لبياناتها "
                "(شبكة/حظر). هذا <b>ليس</b> «لا توجد أسهم»: المسح لم يعمل أصلًا. "
                "أعِد التشغيل لاحقًا.\n\n" + bot.FOOTER)
    if not results:
        return ("🗓️ <b>مرشحو ناسداك (قوي فنياً + إعلان أرباح قريب)</b>\n"
                f"التاريخ: {today}\n\n"
