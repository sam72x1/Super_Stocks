...
def explosion_index(high, low, rise_pct=EXPLOSION_RISE_PCT,
                    base_bars=EXPLOSION_BASE_BARS):
    """🎯 D2: فهرسُ **أوّل** جلسةٍ يبلغ فيها `high` نسبة `rise_pct`% فوق **أدنى
    `low`** في `base_bars` جلسةً **سابقة**. يرجّع `None` إن لم يقع.
    ...
    ⚠️ **الأولى لا الأكبر** (‏D2-④): فيصل يشتري قبل الحركة الأولى لا بعد أكبرها."""
    n = len(high)
    if n <= base_bars:
        return None
    for i in range(base_bars, n):
