...
EXCLUDED_BY_OWNER = {
    ...
    # 🔴 **قرارُ المالك 2026-08-06 — «خلّنا على 25 فقط، ما عندي مجال للمخاطرة أبدًا»:**
    #    هؤلاء الثلاثة **اختارهم فارزُنا لا فيصل**. وسمُهم في `borrow_labelled_set.md`
    #    **`bot_selected`** حرفيًّا، و**صفرُ ذكرٍ لهم في كلّ ملفّات مصادر فيصل**
    #    (‏فحصٌ مباشر: `APVO`/`CMTL`/`KLXE` = 0 إشارة · مقابل `JZ` = 37 و`ONCO` = 18).
    #    ⇒ **دائريّة:** نسأل «ما معاييرُ فيصل؟» وثلاثةٌ من الأجوبة صادرةٌ عن بوّاباتنا
    #    نفسِها، فتُبارك الأداةُ ما بنيناه لأنها مبنيّةٌ جزئيًّا عليه. وهو **صنفُ `HTZ`**
    #    المُستبعَد سلفًا للسبب عينه.
    #    ⚖️ **والقرارُ اتُّخذ قبل رؤية أيّ رقمٍ من ذراع الـ25** (التشغيلة كانت جارية
    #    ولم يُقرأ مُخرَجُها) ⇒ ليس معايرةً بعد النظر.
    #    🔒 **والاستبعادُ بنيويٌّ هنا لا وسيطُ تشغيل** — وإلّا عاد 28 صامتًا في أوّل
    #    تشغيلةٍ تُنسى فيها `ENV_SYMBOLS`. **يُقتَل الصنفُ لا الحالة.**
    "APVO": "قرارُ المالك 2026-08-06 — bot_selected: اختاره فارزُنا لا فيصل (دائريّة)",
    "CMTL": "قرارُ المالك 2026-08-06 — bot_selected: اختاره فارزُنا لا فيصل (دائريّة)",
    "KLXE": "قرارُ المالك 2026-08-06 — bot_selected: اختاره فارزُنا لا فيصل (دائريّة)",
}
