...
⑪ **👀 تحت المتابعة — مجتمعٌ ثالث** (أمرُ المالك 2026-09-26 بعد التقريرين: «فيه سهمين اذكر ريتو و واحد معه ارتفعوا نسبة
   ...
   (`S.NEAR_WATCH_FILE` بالاسم) في أحدث لقطةٍ قبل 09:30 نيويورك **خارج** القائمة والارتداد · الفلوتُ من **المدخل نفسِه** إن
   حمله (مخزنُ فلوت «تحت المتابعة» منذ 2026-09-26 · أمرُ «احفظ فلوت تحت المتابعة») وإلّا من لقطة ذاكرة البوت (`S.COMPANY_FILE`)
   قبل الجلسة نفسِها · **والمتاحُ مجهولٌ بالبناء** (لا يُخزَّن لهم) ⇒ مَن استوفى الثلاثةَ المعلومة (RSI ·
    ...
    for d in days:
        ...
        for s, e in nw_by[d].items():
            if s in nw_union:
                rsi = WW.rsi_at(adj_by.get(s) or [], c)
                px = WW.close_at(raw_by.get(s) or [], c)
                #    👀🏢 فلوتُ المدخل نفسِه أوّلًا (مخزنُ «تحت المتابعة» اليوميّ منذ 2026-09-26 · ياهو `floatShares` · من لقطة ما
                #    قبل الجلسة نفسِها ⟵ point-in-time) ثمّ ذاكرةُ البوت كما كانت (قبل المخزن لا مفتاحَ في المدخل ⟵ بت-بت)
                fl = WW._num(e.get("float"))
                if fl is None:
