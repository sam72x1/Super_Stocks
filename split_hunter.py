...
def _stamp(S, session_date) -> None:
    ...
    ⚠️ يُستدعى بعد اجتياز حارس التغطية ونجاح المسح، و**قبل** بوّابة «لا مطابق»:
    يومُ الصمت الشرعيّ تشغيلةٌ ناجحة تمامًا، وهو بالضبط اليوم الذي يجب ألّا يُخلَط
    بالسقوط. وقبل الإرسال أيضًا: الختم يشهد أن **السوق فُحِص**، ورفضُ تلغرام يُبلَّغ
    بقناته الخاصّة (rc=1) فلا يُخفيه الختم.
...
def run(fetch_ext=None):
    ...
    try:
        ...
        sess = None
    _stamp(S, sess)                     # 🔔 ⓿-و: «السوق فُحِص اليوم» — تاريخٌ واحد
    if not rows:
