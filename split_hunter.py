...
def prev_session_date(day, max_back=10):
    """📅 **آخرُ يومِ تداولٍ قبل `day`** — أيّامُ عملٍ حصرًا مع تخطّي عطلات السوق.

    تُستعمل في فرع الاستدراك بـ`session_gate`: أيُّ يومٍ تُرجعه أُغلق افترُه الساعةَ
    ‏20:00 من يومه، وذلك قبل منتصف ليل `day` ⇒ **الشمعةُ مكتملةٌ يقينًا**.

    ⚠️ **والعطلاتُ أفضل-جهد:** `market_calendar` مداه ينتهي `CALENDAR_LAST_YEAR`،
    وخارجَه يبقى تخطّي نهاية الأسبوع وحدَه. **وتعذّرُ الوحدة لا يُسقط الدالّة**
    (فاشلٌ-آمن: نهايةُ الأسبوع وحدَها) — لأن يومًا مُسمًّى خطأً أهونُ من صمتٍ كامل،
    والدِدوبُ الحاسمُ في الصيّاد مرجعُه **جلسةُ البيانات** فيصيب العطلةَ بعدُ.

    دالّةٌ **نقيّة** (بلا ساعةٍ ولا شبكة) — `max_back` صمّامٌ ضدّ حلقةٍ لا تنتهي."""
    import datetime as _dt
    try:
        import market_calendar as _mc                            # noqa: PLC0415
        _hol = getattr(_mc, "HOLIDAYS", set())
    except Exception:                                            # noqa: BLE001
        _hol = set()
    d = day
    for _ in range(int(max_back)):
...
def session_gate(now_utc=None, close_hour=20, allow_backfill=True):
    ...
    ساعة **نيويورك** فتصيب الفصلين ذاتيًّا بلا جدولٍ يدويّ.
      • ‏ET من ‏20:00 فأعلى ⇒ `(True, التاريخ النيويوركيّ الحاليّ)` — **بت-بت كما كان**.
      • ‏ET قبل ‏20:00 ⇒ **أحدثُ جلسةٍ أُغلق افترُها يقينًا** = آخرُ يومِ تداولٍ
        **قبل** اليوم (‏`prev_session_date`) — وكان `(False, None)` = لا مسح.

    🔴🔴 **ولماذا تغيّر الفرعُ الثاني (2026-08-29، تشخيصٌ من سجلّ التشغيلات لا تخمين):**
    كتبتُ «الكرونُ الثاني سيلتقط» وهو **باطلٌ حين يتأخّر الكرونان معًا**. وGitHub
    رفع تأخيرَ المجدولات إلى **‏7-12 ساعة** (مقيسٌ على أربعة workflows) فصار كرونا
    ‏00:13 و01:13 UTC يقعان في **صباح نيويورك** ⇒ `et.hour < 20` ⇒ `(False, None)`
    ⇒ **خمسُ أدواتٍ ماتت صامتةً ثلاثةَ أيامِ تداول** (‏`press_radar_state` و
    `split_hunter_stamp` كلاهما عند 2026-08-25) **وكلُّ تشغيلةٍ منها خضراء**.

    ⚖️ **والفرعُ الجديد لا يُرخي شرطًا:** الجلسةُ المُرجَعة تُغلَق افترُها الساعةَ
    ‏20:00 من يومها، وذلك **قبل منتصف ليل اليوم الحاليّ** ⇒ **مُغلقٌ يقينًا بالبناء**
    مهما كانت الساعة. فلا تُقرأ شمعةٌ ناقصةٌ أبدًا — وهو الغرضُ الذي وُلدت له.
    و`allow_backfill=False` يُبقي السلوكَ القديم للاختبار والرفض الصريح.
...
def run(fetch_ext=None, now_utc=None):
    ...
    # 🔒 **مساواةٌ لا ترتيب — وهذا مقصودٌ ومقفول (إقرارٌ مؤرَّخ 2026-08-29):**
    #    جرّبتُ `>=` لأمنعَ إعادةَ تسليمٍ لجلسةٍ أقدم بعد فرع الاستدراك، **فأمسكني
    #    قفلان قائمان — وكانا محقَّين:** الختمُ يُكتب بـ**جلسة البيانات** (السطر
    #    «تاريخ الجلسة من البيانات لا `date.today()`») والدِدوبُ السريع يقارنه
    #    بـ**تاريخ نيويورك** ⇒ **مقياسان تحت كلمةٍ واحدة**، وترتيبُهما بلا معنى.
    #    والحمايةُ من التكرار الرجعيّ مكانُها **الدِدوب الحاسم أدناه** ومرجعُه
    #    جلسةُ البيانات نفسُها — وهو الموصوفُ أصلًا بأنه «الحاسم».
