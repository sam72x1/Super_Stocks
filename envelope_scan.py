...
def load_edges(path: str = None) -> dict:
    ...
    # 🔴 **المسارُ يُحسم وقت النداء لا وقت التعريف** (عيبٌ كشفه قفلُ FAI6، 2026-08-06):
    #    كان `path: str = EDGES_FILE` فيُربَط الافتراضُ **لحظةَ تعريف الدالّة** ⇒ تحويلُ
    #    الثابت لاحقًا (اختبارٌ · مسارٌ بديل) **لا يؤثّر إطلاقًا**. وهو **الصنفُ نفسه**
    #    المُصلَح في `record_rejected_symbols` (‏2026-08-05) — تكرّر في وحدةٍ أخرى.
    #    والسلوكُ عند الإغفال **مطابقٌ حرفيًّا** لما كان (نفسُ الثابت).
    path = EDGES_FILE if path is None else path
    try:
...
def chase_ok(S, sym: str, df) -> bool:
    """🚧 **‏D11 — منع الملاحقة (‏`RECENT_RISE_BLOCK_PCT`).**

    🔴 **سببُ وجودها كان عيبًا مقيسًا:** مفاتيح الإرخاء ثلاثة عشر ومعايير الظرف
    كانت تغطّي اثني عشر ⇒ مفتاحٌ واحد يُرخى بلا معيارٍ يقابله، وهو بعينه
    `RECENT_RISE_BLOCK_PCT` — فكان الظرفُ بلا هذا يلاحق ما ارتفع سلفًا.
    ✅ **الفجوةُ أُغلقت (2026-08-07، قرار المالك «قسها» — فتحُ D11 بإذنه):**
    `gain5` صار معيارَ ظرفٍ مقيسًا من الكاتالوج يُحِلّ حدَّه في
    `RECENT_RISE_BLOCK_PCT` نفسِه ⇒ هذي الدالّة الآن **تُنفِّذ الحدَّ المقيس**
    (تقرأ `CONFIG` الذي حلّ فيه رقمُ فيصل) لا رقمَنا القديم 35 — وتبقى لأنها
    تنفيذُ البوّابة عبر `analyze_ticker` الإنتاجيّ بلا صيغةٍ موازية.
