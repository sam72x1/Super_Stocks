...
🔒🔴 **مُغلَقٌ مُنفَّذًا منذ 2026-09-24** («اقفل EDGAR» بالتفويض · `edgar2_result.md §③`): الحكمُ «فشل»
   (‏+2.28 نقطة والحدّ 15) ⇒ `main` تخرج **‏8** قبل أيّ عمليّة ما لم يكن `EDGAR_REOPEN` = `1` حرفيًّا.
...
# ═══════════ 🔒 إغلاقُ المحور — «اقفل EDGAR» (‏2026-09-24، بالتفويض الكامل) ═══════════
#   الحكمُ صدر **«فشل»** (‏`edgar2_result.md §②` · التشغيلة 36041963692): الفرقُ ‏+2.28 نقطة والحدّ 15 ·
#   الفاصلان متداخلان · و`EG6` ‏−0.27 ⇒ إغلاقٌ **يُنفَّذ لا يُكتَب** بنمط `T-WAVES §⑨`/`T-HRT §⑦`:
#   الدراسةُ تخرج ‏8 **قبل** أيّ سطرٍ أو قراءةِ مُدخَلٍ أو نداءِ SEC.
AXIS_CLOSED = True
REOPEN_ENV = "EDGAR_REOPEN"
CLOSED_RC = 8            # مميَّزٌ عمدًا عن 0/2/3/4/5/**6 «لا حكم»**


def _closed_now() -> bool:
    """أمُغلَقٌ الآن؟ — المفتاحُ يُقرأ **وقت النداء** لا وقت الاستيراد.

    🔒 **والإقرارُ `1` حرفيًّا** (سابقةُ `HRT_REOPEN`/`WAVES_REOPEN`): مُدخَلُ الـworkflow افتراضُه `0`
    ويصل البيئةَ نصًّا غيرَ فارغ ⇒ لو فُحص «غيرُ فارغ» **لارتفع الإغلاقُ في كلّ تشغيلة** وهو يبدو مُغلَقًا."""
    return AXIS_CLOSED and (os.environ.get("EDGAR_REOPEN") or "").strip() != "1"


def closure_notice() -> list:
    """نصُّ الإغلاق — **دالّةٌ نقيّة** ليُقفَل مضمونُها لا شكلُها."""
    return [
        "🔒🔴 **محورُ «إيداعُ SEC الليليّ قبل انفجار البريماركت» (`T-PRE-EDGAR-2`) مُغلَق** — "
        "«اقفل EDGAR» (‏2026-09-24 · بالتفويض الكامل) · `edgar2_result.md §③`.",
        "",
        "⚖️ **السبب — بالأرقام لا بالرأي** (`edgar2_result.md §②` · التشغيلةُ الحاكمة ‏36041963692):",
        "   • **«فشل»:** EG1 ‏29/513 = 5.65% مقابل EG2 ‏20/593 = 3.37% ⇒ الفرقُ ‏+2.28 نقطة والحدّ ‏15 · "
        "والفاصلان متداخلان · وEG6 العنقوديّ ‏−0.27.",
        "   • **والأنبوبُ سليم:** البوّاباتُ الأربعَ عشرة عبرت · صفرُ مجهول · صفرُ حجب · والتوقيتُ B "
        "(‏22,125 إيداعًا) ⇒ لا عطبَ يُصلَح فيُعاد القياس.",
        "",
        "🔓 **ولا يُعاد فتحُه إلّا بثلاثةٍ معًا (`edgar2_result.md §③`):**",
        "   ① **مصدرٌ أو محورٌ جديدٌ لم يُقَس** (نموذجٌ خارج FORMS كـ6-K · مصدرُ أخبارٍ غيرُ EDGAR · "
        "أو سنواتٌ أخرى بعقدٍ يُعلن سببَه) — وإعادةُ الدراسة نفسِها أو خفضُ الحدّ 15 ليست محورًا.",
        "   ② **تسجيلٌ مسبقٌ جديد** — العقدان وملحقُهما مدموجةٌ لا تُعدَّل، ولا يُرخى حدٌّ رُئيت قيمتُه.",
        "   ③ **إذنُ المالك**.",
        "   ⚖️ والمفتاحُ `EDGAR_REOPEN=1` **إقرارٌ بالثلاثة لا التفافٌ عليها** — ويُقرأ `1` حرفيًّا.",
    ]
...
def main(get=None, sleep=None, paths=None, cmap=None) -> int:
    ...
    # 🔒 الحارسُ **قبل أيّ سطرٍ أو قراءةِ مُدخَلٍ أو نداءِ SEC** («اقفل EDGAR» 2026-09-24).
    if _closed_now():
