...
def _lock_arms():
    ...
    # 🔓 **إقرارٌ مؤرَّخ 2026-08-19:** أحجامُ العيّنة صارت **نسبيّةً لأرضية
    #    الإنتاج الحيّة** — الأحجامُ المغروسة (‏12,000) بُنيت على أرضية $30 ألفًا،
    #    ولمّا اعتمد المالكُ $15 ألفًا صار تراكميُّ الدقيقة 9 (‏≈$25.6 ألفًا)
    #    **فوق** الأرضية فتُطلق `R1` عندها لا عند 10. الآن دقيقتا التمهيد
    #    ‏≈0.85× الأرضية (تحتها) والقافزةُ ‏≈1.5× (فوقها) ⇒ العيّنةُ تفرّق
    #    عند أيّ أرضيةٍ يختارها المالك.
    _fl = float(bot.LIQ_MIN_USD)
    _v89 = int(round((0.85 * _fl - 500.0) / 2.0909))
    _v10 = int(round(1.5 * _fl / 1.0927))
    flat = [_b(i, 1.0, 1.0, 1.0, 1.0, 500) for i in range(8)]
    climb = flat + [_b(8, 1.0000, 1.0300, 1.0000, 1.0300, _v89),
                    _b(9, 1.0300, 1.0609, 1.0300, 1.0609, _v89),
                    _b(10, 1.0609, 1.0927, 1.0609, 1.0927, _v10),
