...
def probe(sym, day, bars, prev_c):
    ...
    # 🔴 **تهيئةٌ ميتةٌ عمدًا — بوّابةُ `F821` (2026-08-28، أمرُ المالك «صلّح
    #    اللنت»):** الاسمان يُقرآن في `seg.append` **لفظيًّا قبل** إسنادهما،
    #    فيَسِمهما ruff «غيرَ معرَّفين» وتُحمَّر البوّابة — **والكودُ سليمٌ وقتَ
    #    التشغيل**: القراءةُ محروسةٌ بـ`cur is not None`، و`cur` لا يُسنَد إلّا
    #    داخل الجولة التي تُسنِدهما في نهايتها بلا فرعٍ يرمي بينهما ⇒ **قراءةٌ
    #    قبل الإسناد مستحيلة**. ⚖️ **والتهيئةُ لا تُقرأ أبدًا** — مُبرهَنٌ على
    #    ‏60,004 حالة (فارغةٌ · كلُّها قبل المِرساة · صفٌّ واحد · شرائحُ كثيرة)
    #    بصفرِ تفرّقٍ وصفرِ انهيار ⇒ **مُخرَجُ الأداة بت-بت وأرقامُ `HUIZ`
    #    المنشورةُ لا تتحرّك.** 🔒 مقفولٌ `LNT1`-`LNT3`.
    last_c, last_t = None, None
...
def _tier_pnl(out, day):
    ...
    🔴 **تصحيحٌ مؤرَّخ 2026-08-23:** كتبتُ أوّلًا أن «بلا كرت» **لا تدخل
    المقام** — **والكودُ يُدخلها، والكودُ هو الصحيح**: كرتُ `M5` يصل
    المالكَ فعلًا ولو كُسر قاعُ المِرساة قبل الدقيقة الخامسة (شرطُ إطلاقه
    اكتمالُ الخمس لا سلامةُ القاع)، فإخراجُها يُجمّل الحصيلة. ⇒ تدخل
    المقامَ **وتُطبَع بعددها** ليطرحها القارئُ إن شاء، ومعها حدُّ الصدق
    المدوَّن (`tier_days_result §⑤`): **دخولُها ورقيٌّ لا يُشترى**."""
    ...
    print("   ⚠️ وعمودُ «بلا كرت» **داخلٌ في `ن`** (كرتُ M5 يصل فعلًا) — "
          "ودخولُه ورقيٌّ لا يُشترى، فاطرحه إن شئت.")
    print("   🔴 وهذي القاعدةُ **بلا هدفِ ربحٍ إطلاقًا**: تُمسك من الكرت حتى "
          "يُكسَر القاعُ أو ينتهي اليوم ⇒ **ليست ربحيّةَ البوت**.")
    return out
