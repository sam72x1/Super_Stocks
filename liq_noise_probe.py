...
def _lock_v4(fetch=None):
    ...
    # 🐞 **عيّنةٌ رابعةٌ أضافتها طفرةٌ نجت** (‏`M7`: حذفُ قمّةِ الماء من `V4` في
    #    المِجَسّ **مرَّ أخضر**): الثلاثةُ أعلاه لا تفرّق على طبقة القمّة أصلًا —
    #    `fade` شموعُها **حمراء** فيكتمها **الاتجاهُ** قبل أن يصل الدورُ للقمّة،
    #    و`big` حدثُها واحدٌ بلا تحديثات. ⇒ هذي **خضراءُ صاعدةُ الإغلاق وكلُّها فوق
    #    الأرضية لكنها متناقصةُ السيولة** (‏45,150 ⟶ 40,320 ⟶ 35,420) فتفرّق
    #    **بالقمّة وحدها**: معها إشعارٌ واحد، وبلاها ثلاثة.
    #    🧭 «القفلُ لا يحرس إلّا بقدر ما تُفرِّق عيّنتُه» — والطفرةُ هي التي كشفته.
...
def main():
    ...
    # 📋 **جدولٌ وصفيٌّ أُضيف بعد الأرقام — ولا يغيّر معيارًا** (‏§⑤ يسمح بأسطر
    #    عرض): المالكُ يسأل عن `$WFF` بعينه («ضيع علينا ربح فوق 150٪») فيُطبَع لكلّ
    #    متحرّكٍ **متى** أطلقت `V0` ومتى `V4` وأين كانت القمّة. **قراءةٌ لا حكم.**
    print("\n" + "=" * 62)
    print("②-ب لكلِّ متحرّكٍ على حدة (وصفيٌّ — لا يدخل قاعدةَ القرار)")
    print("=" * 62)
    print("     السهم | صعودُ الجلسة | دقيقةُ V0 | دقيقةُ V4 | دقيقةُ القمّة | "
          "أسبقيّةُ V4")
    for s in sorted(movers, key=lambda x: -refs[x]["rise_pct"]):
        k0 = res["V0"]["m1"].get(s)
        k4 = res["V4"]["m1"].get(s)
        hi = refs[s]["high_i"]
        lead = (hi - k4) if k4 is not None else None
        print(f"     ${s:<5} | ‏+{refs[s]['rise_pct']:>7.1f}% | "
              f"{('—' if k0 is None else k0):>8} | "
              f"{('—' if k4 is None else k4):>8} | {hi:>12} | "
              f"{('—' if lead is None else str(lead) + ' دقيقة'):>10}")
    print("     (الأرقامُ فهارسُ دقائقَ داخل النافذة المجلوبة — الفرقُ هو المعنى.)")
