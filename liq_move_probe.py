...
FROZEN = {"ضخمةٌ داخلة": 1, "صغيرةٌ نسبيّة": 0, "سلسلةٌ خابية": 1,
          "خضراءُ متناقصة": 1, "TENX بلا تجاوب": 0,
          "هادئٌ طويلٌ ثم ثلاثٌ صغيرة": 0}


def _lock_prod():
    """🔒 `LOCK-FROZEN` — مُخرَجُ `A1/B1` **مُجمَّدٌ** على نشرة `liq_move_result.md`
    (ستُّ عيّناتٍ مفرِّقة) ‏+ ثلاثةُ شواهدِ تفريق. **ولا يدّعي مطابقةَ الإنتاج** —
    تلك تُقفَل سلوكيًّا على `liq_stage_events` نفسِها في السويّة."""
    def _b(t, o, h, l, c, v):
    ...
    for name, bars in (("ضخمةٌ داخلة", big), ("صغيرةٌ نسبيّة", small),
        ...
        # 🔴🔓 **إقرارٌ مؤرَّخ 2026-08-17 — ادّعاءُ «مطابقةِ الإنتاج» سُحب من هنا
        #    أيضًا، بنيويًّا لا مؤقّتًا:** الإنتاجُ تغيّر **مرّتين في يومٍ واحد**
        #    (بوّابةُ التجاوب ثم إشعارُ `M0` قبل الإغلاق)، وفي كلّ مرّةٍ كان على
        #    المِجَسّ أن يلحق — **وأيُّ نسخةٍ تدّعي المطابقةَ تتفرّق عاجلًا وتقول
        #    «مطابق» كذبًا**. ⇒ **لا نسخةَ تُطارد الإنتاج:** هذا المِجَسُّ **مُجمَّدٌ**
        #    ووظيفتُه **إعادةُ إنتاج أرقام `liq_move_result.md`** بت-بت،
        #    **وهويّةُ الإنتاج تُقفَل سلوكيًّا على `liq_stage_events` نفسِها** في
        #    السويّة (‏`LS1`-`LS24`) — وهو أقوى شكلٍ للقفل لأنه بلا وسيط.
        mine = replay(bars, A_ARMS["A1"], B_ARMS["B1"])
        want = FROZEN[name]
        same = len(mine) == want
        notes.append(f"{name}: A1/B1={len(mine)} مُجمَّد={want} "
                     f"{'✅' if same else '❌'}")
        ok = ok and same
