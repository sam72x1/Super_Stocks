...
EARLY_CLOSES = {
    ...
    # ---- 2023-2025 (أُضيفت 2026-09-23 — بناءُ `T-WAVES` · **مدًى للخلف لا للأمام**) ----
    # 🔴 **السببُ مقيس:** مُدّت العطلاتُ لهذي السنوات يوم 2026-08-21 **ولم يُمَدّ هذا
    #    الجدول** ⇒ `session_info` كانت ترجع «regular» لأيّام الإغلاق المبكّر الثمانية ·
    #    و`ah_scan.calendar_years()` تَعُدّ السنواتِ «مُتحقَّقة» من العطلات وحدَها. أمسكه
    #    بناءُ `T-WAVES` قبل أيّ رقم (العقد يستبعد الإغلاقَ المبكّر بهذي الدالّة بالاسم).
    # 📌 **المصدر:** جدولُ NYSE/Nasdaq الرسميّ · **ومصادقةٌ مستقلّة:** مكتبةُ
    #    `exchange_calendars` (‏XNYS وXNAS كلتاهما) تُعطي **هذي الثمانية بحذافيرها**
    #    وكلُّها 13:00، وأيّامَ تداولٍ 250/252/250 = أيّامُ `NL_UP` في `link100_probe`.
    #    (‏24 ديسمبر 2023 وقع أحدًا ⇒ لا إغلاقَ مبكرًا له.)
    # 🔒 **خاملٌ على الإنتاج بنيويًّا** كمدّ العطلات: المسارُ الحيّ يسأل عن اليوم (2026+).
    "2023-07-03": 13 * 60, "2023-11-24": 13 * 60,
    "2024-07-03": 13 * 60, "2024-11-29": 13 * 60, "2024-12-24": 13 * 60,
    "2025-07-03": 13 * 60, "2025-11-28": 13 * 60, "2025-12-24": 13 * 60,
}

# 🗓️ عددُ أيّام الإغلاق المبكّر لكلّ سنةٍ مثبَّتًا — **حارسُ اكتمالٍ** مرآةُ
#    `HOLIDAY_COUNT_BY_YEAR`: سنةٌ غائبةٌ هنا أو عددٌ مختلف ⇒ أداةُ القياس تتوقّف
#    بدل أن تَسِم نصفَ جلسةٍ «نظاميّةً» صامتًا (وهو بعينه ما وقع قبل 2026-09-23).
EARLY_CLOSE_COUNT_BY_YEAR = {2023: 2, 2024: 3, 2025: 3, 2026: 2, 2027: 1}
