...
def year_days(year: str) -> list:
    """أيّامُ تداول السنة — **عطلةُ نهاية الأسبوع تُستبعَد هنا**.

    🔴 `market_calendar.is_trading_day` تفحص **العطلَ الرسمية وحدَها** ونصُّ
    دوكسترنغها يقول ذلك صراحةً («لا يفحص عطلة نهاية الأسبوع — الكرون أيام العمل
    فقط») ⇒ الاتّكالُ على اسمها وحدَه أعطى **‏355 «يومَ تداول» في 2023** فأسقط
    `V-L1` بتغطيةِ ‏70.4% — **وضعُ الجدوى هو الذي كشفه قبل إنفاق أيّ ساعة**.
    ⇒ يُجمَع الشرطان: ليس سبتًا ولا أحدًا **و**ليس عطلةً رسمية (‏`LKA10`)."""
    d = dt.date(int(year), 1, 1)
    end = dt.date(int(year), 12, 31)
    out = []
    while d <= end:
        if d.weekday() < 5 and is_trading_day(d.isoformat()):    # بالاسم
