...
def tier_of(row):
    ...
    🥇 **وبأمره «اعتمد v1» (‏2026-08-26، بعد `T-TIER-V3`) صار الاشتقاقُ
    الاتحادَ**: قوي = العدّادُ فأكثر **أو** `j1` المخزَّن (السجلُّ يحمل
    `j1` بوليانًا منذ ولادته ⇒ إعادةُ الاشتقاق بأثرٍ رجعيٍّ ممكنةٌ بلا
    إعادة حساب — وصفوفُ `DAIC` ‏08-24/08-25 تنقلب قويًّا بها).

    ⇒ **مطابقٌ لـ`liq_tier` النافذة** — مقفولٌ **سلوكيًّا** في السويّة على
    القيم صفر إلى أربع وعلى صفَّي تكافؤ `J1`."""
    g = row.get("green")
    if g is None:
        return "غير مصنَّف"
    try:
        ...
        return "غير مصنَّف"
    if g >= _STRONG_MIN or bool(row.get("j1")):
        return "قوي"
    return "ضعيف" if g <= _STRONG_MIN - 2 else "متوسط"
