...
### 19. مراجعة Codex الخامسة (خصومية على `966fe1b`) — NO-SHIP بـ3 ملاحظات (كلها نُفِّذت)
حكم Codex: الجذور ونص تلغرام **غير ممسوسة** وscan_ignition **بلا جلب NBBO تزامني** — لكن الطبقة
**لم تكن instrumentation-only فعليًّا** في ثلاثة مواضع. الإصلاحات (818 اختبار · خروج 0 · لا LOGIC_VERSION):

| # | الملاحظة (Codex) | الحلّ |
|---|---|---|
| **1 (high)** | **فشل القياس يقتل الإنتاج**: مقطع `close` كان يتحقّق من handoff/manifest ثم `SystemExit(2)` **fail-closed** ⇒ خلل في المسجّل/الرفع يمنع **كل تنبيهات مقطع الإغلاق**. سلامة القياس يجب أن تقرّر **أهلية الجلسة للتحليل** لا **استمرار الرادار**. | **fail-open للإنتاج**: أُزيل `SystemExit` و`IGNITION_HANDOFF_STRICT` من مسار الرادار. الفشل ⇒ نواصل المسح بلا استعادة دِدوب (تكرار تنبيه محتمل ≪ فقد كل التنبيهات) + يُسجَّل `handoff_verify_failed` بالميتا. **الرفض يبقى حيث يجب**: الassembler يتحقّق من السلسلة والمدقّق يرفض `session_complete` بـ`manifest_chain_ok=False` فلا تُعدّ نحو 5/20. قفلان جديدان. |
| **2 (high)** | **الإيقاع المطلق (P1-2) تغييرُ سلوك تنبيه**: بدأ المسح كل `interval` بدل `interval+scan` ⇒ تتغيّر أوقات المسح ⇒ تتغيّر الإشارة المرصودة ووقت ختم الدِدوب. | **رجوع لسلوك الإنتاج قبل الفرع حرفيًّا**: `time.sleep(interval)` بعد المسح. القياس **يرصد** التأخّر/التجاوز سلبيًّا (`schedule_lag_ms`/`loops_over_interval_ratio`) ولا يصحّحه. قفل: لا `_next_tick` بالحلقة. |
| **3 (med)** | **`trace=None` ليس صفر عمل**: الحمولة كسولة نعم، لكن لكل سهم يُقرأ الرمز ويُبنى lambda ويُنادى `_emit_trace` ⇒ ادّعاء «بت-بت» غير دقيق. | كل موقع بثّ خلف `if _tr:` (`_tr = trace is not None` مرة واحدة) ⇒ **صفر lambda وصفر نداء** عند None. اختبار توصيف: `trace=None` ⇒ 0 نداء `_emit_trace` ونفس المخرجات · قفل: عدد `if _tr:` = عدد مواقع البثّ. |

**⚠️ ملاحظة توتّر مقصود:** الملاحظة 1 تُلغي «close fail-closed» التي طلبتها مراجعة Codex الرابعة (P0-4).
التوفيق: **الmanifest يبقى fail-closed للقياس** (سلسلة التحقّق + رفض المدقّق) لكن **لا يوقف التنبيه** —
هذا هو مقصد P0-4 (منع جلسة فاسدة من العدّ) بلا ثمن إنتاجي.

#### 19.1 حلقة المراجعة الخصومية (12 تمريرة) — المبدأ الحاكم
Codex واصل بعد الثلاث الأولى بسلسلة موانع من **نفس المحور**: «هل يستطيع فشلُ قياسٍ أن يُسكت الإنتاج؟».
نُفِّذت كلها (834 اختبار · خروج 0):

| # | المنفذ | الحلّ |
|---|---|---|
| ب | `close_segment` معلّق بـ`needs`+تنزيل artifact إلزامي ⇒ فشل القياس يلغي تنبيهات 195د | `if: always()` + تنزيل `continue-on-error` + قفل بنيوي |
| ج | `import ignition_measurement` على مستوى الوحدة ⇒ انكساره يقتل السكربت قبل `main()` | استيراد كسول داخل فرع E2 المحمي + اختبار ImportError مُجبَر |
| د | خطّافات المسجّل تُنادى بلا حماية (استثناء في `telegram_attempt` يمنع `send_telegram`) | `_SafeRecorder`: حدّ فاشل-آمن عند نقطة النداء + تعطيل دائم عند الفشل |
| هـ | خطّاف **معلّق** (deadlock/قرص) لا يرمي أبدًا ⇒ يخنق المسح | طابور محدود (20k) + خيط عامل · إسقاط عند الامتلاء (+`measurement_dropped`) · ختامي بمهلة 30ث |
| و | الاستيراد وبناء المسجّل (قرص) على خيط الإنتاج قبل الحلقة | تهيئة في خيط daemon · الرادار يبدأ فورًا بـ`trace=None` · التحاق لا-حاجب |
| ز | سباق جاهزية (`not _rec_box`) ⇒ المسجّل لا يلتحق أبدًا = **ضياع القياس بصمت** | `threading.Event` يُضبَط في `finally` + `is_set()` |
| ح | ميتا القياس (`_wl_content_sha256`: استيراد+تقنين+هَشّ) قبل الخيط | كل الميتا داخل `_init_recorder` |
| ط | مسار التحديث: `_fetch_head_sha` (git subprocess) والهَشّ يُقيَّمان **كوسائط** بالإنتاج | `_SafeRecorder.submit(fn)` يؤجّل العمل كاملًا للعامل |
| ي | المهمّة المؤجَّلة تقرأ حالة عامّة متأخّرًا ⇒ **provenance كاذب** عبر جلبتين | لقطة غير قابلة للتغيّر لكل جلبة + اختبار عامل متوقّف عبر جلبتين |
| ك | قراءة `.git/FETCH_HEAD` = I/O قياسي بلا مهلة على خيط التنبيه | أُسقط التقاط الـcommit: `WL_COMMIT_UNRESOLVED` صراحةً · البرهان = هَشّ المحتوى |

**المبدأ المستخلص:** *سلامة القياس تقرّر أهلية الجلسة للتحليل، لا استمرار الرادار.* الرادار لا يعتمد
القياس في أي مسار: لا استيرادًا، لا بناءً، لا خطّافًا، لا I/O، لا حتى في الـworkflow.

**⛔ ما لم يُنفَّذ (وقفة مقصودة — قرار المالك):**
- **تمريرة 12 — «ابنِ حمولة الـtrace على العامل لا على خيط المسح»:** لم تُنفَّذ. الحمولة تُبنى من `s`/`sig`/
  `bars` التي **تعدّلها الحلقة بعدها** (`ignition_alert`/`last_price`/تبديل القائمة) ⇒ تقييمها متأخّرًا =
  تسجيل حالة مُعدَّلة = **نفس صنف سباق الـprovenance الذي رفضه Codex في التمريرة 11**. البديل (نسخ عميق
  قبل الطابور) = عمل **أكثر** على خيط التنبيه. الوضع الحالي = الحدّ الأدنى للقطة صادقة: بناء قاموس في
  الذاكرة (بلا I/O · بلا قفل) ثم `put_nowait`. **وبـ`E2_MEASUREMENT` مطفأ (الإنتاج الافتراضي) المسار
  بت-بت.** أي تغيير هنا يمسّ صحة البيانات — يحتاج قرارك.
- **حسم commit القائمة المحدَّثة:** يتطلّب إمّا I/O قياسيًّا على خيط التنبيه، أو نقل **تحديث القائمة كلّه**
  لخلفية محدودة = **تغيير سلوك إنتاجي** (الرادار قد يمسح على قائمة أقدم) — خارج نطاق instrumentation-only.

---
**الخلاصة:** الرادار صار تجربة prospective كاملة القياس **دون أن يتغيّر تنبيه واحد** (وبعد مراجعة Codex
الخامسة: بلا أي أثر إنتاجي حتى في الإيقاع أو مسار trace=None). الخطوة التالية = تراكم ≥5 جلسات كاملة
(E2-A) ثم E2-B الوصفي — بموافقة المالك، لا تطبيق تلقائي.
