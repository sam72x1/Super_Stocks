...
def _ready_card(i: int, r: dict) -> list:
    ...
    # 🛑 وقف القاع المعتمد (أمر المالك 2026-08-27 «اعتمد وقف القاع» بعد
    # القياس الثلاثي T-GOLD-ENTRY/T-SLIP/T-SLIP-NBBO: خطة P1 بوقفٍ عند
    # القاع نفسه +0.576R على العرض القائم مقابل +0.174R لوقف 7% تحته —
    # والانزلاق عنده أدنى لأنه مستوى فيه طلبات قائمة). الدفعات لم تتغيّر.
    seg.append(f"   🟣 الطلبات قرب القاع ${p.get('press_low')} · "
               f"🛑 الوقف = القاع نفسه ${p.get('press_low')} (وقف القاع المعتمد)")
    if p.get("imp_head"):
...
def build_alert(rows, session_iso: str) -> str:
    ...
    if "🩸" in body:                    # شرحُ الكنس مرّةً واحدة — بسنده المقيس
        # ⚖️ حدُّ صدق §⑯ (2026-08-15، إنفاذًا لنصّ P-X3 «إن كُسر يُقيَّد»):
        # على 3 سنوات تفوّق المكنوسُ في 2023 و2024 وانقلب بفارقٍ ضئيل في 2025
        lines.append("🩸 <b>كُنس بعد حفظ</b> = قاعٌ صمد ثم كُسر بقاعٍ أطزج "
                     ...
                     "وقياسُ 3 سنوات: تفوّق في سنتين وانقلب بفارقٍ ضئيل في "
                     "2025 — ترتيبُ انتباهٍ لا حكم.")
    ...
    if "🟣" in body:
        lines.append("🟣 <b>الطلبات</b> = منطقةُ الشراء عند القاع · 🛑 "
                     "<b>وقف القاع</b> = القاعُ نفسُه لا تحته (اعتماد "
                     "المالك 2026-08-27 — القياسُ الثلاثيّ أعطاه "
                     "+0.576R على العرض القائم مقابل +0.174R لوقفِ "
                     "7% تحت القاع، وانزلاقُه أدنى لأن القاع مستوى "
                     "فيه طلباتٌ قائمة).")
...
def run(now_utc=None, fetch_hist=None, sender=None, state_path=STATE_FILE,
    ...
    # 🩺 **أرضيةُ التغطية** (خطة 015 · 2026-08-15) — الرادارُ كان **الوحيدَ** من
    # خمسة صيّادين بلا هذا الحارس: خنقُ ياهو يُرجع إطاراتٍ فارغة فيخرج «‏📭 لا
    # مطابق» **ويختم الجلسة**، فيرى الكرونُ الثاني الختمَ وينصرف ⇒ **ليلةٌ بلا
    # رسالةٍ وبلا إشارة عطل** = عينُ «الصمتُ لا يفرّق: لا مرشّح عن لم يُفحَص».
    # الآن: تحت الأرضية ⇒ **خروجٌ غيرُ صفريّ بلا ختمٍ ولا حصاد** فيُعيد الكرونُ
    # الثاني المسحَ كاملًا. (نفس `MIN_COVERAGE_PCT` وصياغةِ الصيّادين الأربعة.)
    cov = 100.0 * (len(pool) - failed) / max(1, len(pool))
    if cov < MIN_COVERAGE_PCT:
        _log(f"⛔ تغطية ناقصة: {len(pool) - failed} من {len(pool)} رمزًا "
             f"({cov:.0f}%) — أقل من الأرضية {MIN_COVERAGE_PCT:.0f}%، فلم "
             f"تُفحَص البِركة. لا ختم ولا حصاد (الكرون الثاني يعيد).")
        return 1
