...
def quote_scale_mismatch(ask, bar_close, tol):
    """🔴 هل الاقتباس والشمعة **بمقياسَي سعرٍ مختلفين**؟ (عيب التقسيم — Codex §④)

    الشموع تُطلَب `adjusted=true` والاقتباسات تأتي **بلا تعديل**؛ فسهمٌ مقسَّمٌ عكسيًّا
    بعد تاريخ الحدث يجعل الشمعة بمقياسٍ والاقتباسَ بآخر، ثم يُقارَن `ask` بوقفٍ وهدفٍ
    مشتقَّين من المعدَّل ⇒ إسقاطٌ صامت أو `R` شاذّة.

    القاعدة: المقارنة **لحظيّة** (اقتباسُ الدخول مقابل إغلاق شمعة الزناد نفسها)،
    فالفارق المشروع بينهما جزءٌ من سبريدٍ ودقيقةٍ واحدة — وأيُّ تباعدٍ يتجاوز `tol`
    اختلافُ **مقياس** لا حركةُ سوق.

    🔒 **فاشلة-آمنة نحو القبول:** قيمةٌ غائبة/تالفة/غير موجبة ⇒ `False` (لا نرفض
    بالظنّ؛ تعذّرُ القياس ليس دليلَ خلل). دالّة **نقيّة**."""
    try:
        a, c = float(ask), float(bar_close)
    except (TypeError, ValueError):
        return False
    if a != a or c != c or a <= 0 or c <= 0:          # ⚠️ NaN ليس None
        return False
    return abs(a / c - 1.0) > float(tol)
