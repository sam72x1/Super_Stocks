...
✅ **وهذا نفسُه يعطي مقياسَ تسليمٍ صادقًا بلا سجلٍّ جديد:** مِرساةٌ **نجت** = حاضرةٌ
في آخر لقطةٍ تحمل **يومَها** · ومِرساةٌ **اختفت قبل نهاية يومها** = كُتمت. فيُفصَل
«أُطلق» عن «وصل» من الحالة نفسِها.
🔴🔴 **وهذا تصحيحٌ مؤرَّخ 2026-09-20 لعيبٍ مقيسٍ في صياغتي الأولى:** كانت تقرأ
«باقيةٌ في **آخر لقطةٍ من المدى كلِّه**»، والحالةُ **تُفرَّغ عند تبدّل اليوم** ⇒
كلُّ مِرساةِ يومٍ ماضٍ تُقرأ «مكتومة» **ولو سُلِّمت**. ظهر الأثرُ بالرقم: على مدًى
من ‏13 يومًا أعطى القديمُ **‏23** والصادقُ أضعافَه. **والقديمُ يبقى مطبوعًا بوسمه**
(نقلٌ لا حذف) ولا يُحكَم به.
...
3. **«نجت» تقريبٌ للتسليم لا يقينُه:** آخرُ لقطةٍ ليومٍ قد تقع أثناء جلسةٍ حيّةٍ
   فتُقرأ مِرساةٌ قائمةٌ «ناجية» وهي بعدُ لم تُحسم — **وهذا يخصّ اليومَ الأخيرَ
   وحدَه** بعد تصحيح 2026-09-20.
        ...
        for k, v in st.items():
            ...
            if cur is None:
                ...
                if row["trig"] and not cur["trig"]:
                    cur["trig"] = row["trig"]
    # ✅ «باقيةٌ في آخر لقطة» = حاضرةٌ في اللقطة الأخيرة من المدى كلِّه.
    # 🔴🔴 **وهذا مقياسٌ يكذب على الأيّام السابقة** (عيبٌ مقيسٌ 2026-09-20): الحالةُ
    #    تُفرَّغ عند تبدّل اليوم، فمِرساةُ يومٍ ماضٍ تغيب عن اللقطة الأخيرة **ولو
    #    نجت من بوّابة المضارب** ⇒ تُقرأ «كُتمت» وهي سُلِّمت. فأُضيف المقياسُ
    #    الصادق **`alive_eod`**: الحضورُ في **آخر لقطةٍ تحمل يومَ المِرساة نفسَه**
    #    — والقديمُ يبقى مطبوعًا بوسمه فلا يُحذف رقمٌ نُشر.
    def _keys_of(_st):
        out = set()
        if isinstance(_st, dict):
            for k, v in _st.items():
                if k.startswith(PREFIX) and isinstance(v, dict) and v.get("anchor_ms"):
                    out.add((k[len(PREFIX):], str(v.get("date") or ""),
                             int(v["anchor_ms"])))
        return out

    snaps = list(snapshots or [])
    last_keys = _keys_of(snaps[-1][1]) if snaps else set()
    # آخرُ لقطةٍ يظهر فيها يومٌ بعينه — تُحسب مرّةً واحدةً لكلّ يوم
    eod_keys, last_idx = {}, {}
    for i, (_ts, _st) in enumerate(snaps):
        for (_sym, _day, _ms) in _keys_of(_st):
            last_idx[_day] = i
    for _day, i in last_idx.items():
        eod_keys[_day] = _keys_of(snaps[i][1])
    for key, row in anchors.items():
        row["alive"] = key in last_keys
        row["alive_eod"] = key in eod_keys.get(key[1], set())
...
def report(res: dict, bad: int = 0, ref: str = "HEAD") -> list:
    ...
    eod_tc = [k for k, v in tc.items() if v.get("alive_eod")]
    eod_r1 = [k for k, v in r1.items() if v.get("alive_eod")]
    L = ["🚦📜 **قراءةُ الزناد الموازي `T-C`**",
         ...
         f"   📬 **الحاكمُ للتسليم — نجت من بوّابة المضارب:** {len(eod_tc)} من "
         f"{len(tc)} موازية = {(len(eod_tc) / len(tc) * 100.0) if tc else 0.0:.1f}%"
         f" · و`R1` {len(eod_r1)} من {len(r1)} = "
         f"{(len(eod_r1) / len(r1) * 100.0) if r1 else 0.0:.1f}%"
         f"  (**حضورٌ في آخر لقطةٍ تحمل يومَ المِرساة نفسَه**)",
         f"   ↳ 🔴 والمقياسُ القديم «باقيةٌ في **آخر لقطةٍ من المدى**» = "
         f"{len(alive_tc)} فقط — **وهو يكذب على الأيّام السابقة** لأن الحالةَ "
         f"تُفرَّغ عند تبدّل اليوم، فيُقرأ المُسلَّمُ القديمُ «مكتومًا». "
         f"يُطبَع للمقارنة ولا يُحكَم به.",
    ...
    for (sym, day, ms), v in sorted(tc.items(), key=lambda x: x[0][2]):
        L.append(f"   🚦 {sym} · {day} · {ms} · مراحل {'/'.join(v['sent']) or '—'}"
                 ...
                 f" · {'نجت' if v.get('alive_eod') else '**كُتمت**'}")
    L += ["   ⚠️ **حدودُ الصدق (مكتوبةٌ قبل أيّ رقم):**",
          ...
          "      ⑤ **«نجت» ليست «وصلت رسالةٌ للمالك»** — بعد البوّابة يبقى"
          " الفلترُ والسقف، والمقياسُ حضورُ المِرساة في آخر لقطةٍ من **يومِها**"
          " لا سجلُّ إرسال."]
    return L
