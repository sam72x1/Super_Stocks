...
def _lock_g0():
    ...
    for nm, bb, arm in (("G1 موضعُ الإغلاق", bars_cp, "G1"),
        ...
        notes.append(f"{nm} تفرّق: G0={a0} · {arm}={a1} "
                     f"{'✅' if good else '❌'}")
        ok = ok and good
    # 🔓 **إقرارٌ مؤرَّخ 2026-08-19:** المالك اعتمد رقمَ `G5` (‏$15,000) في
    #    الإنتاج («حطها بافضل رقم يناسب الاستراتيجية الحالية») ⇒ صار `G5`
    #    يطابق `G0` **بالبناء**، والتفرّقُ الذي كان هنا (‏cum ‏≈$21.6 ألفًا بين
    #    الأرضيتين القديمة والجديدة) **استحالةٌ لا عيب**. القفلُ انقلب إلى
    #    **تكافؤ**: العيّنةُ يلتقطها الاثنان — ولو عادت أرضيةُ الإنتاج يومًا
    #    فوق رقم `G5` عاد التفرّقُ فسقط هذا القفلُ وكشف الانحراف.
    a0_fl = anchor_of(bars_fl, ARMS["G0"])[0]
    a5_fl = anchor_of(bars_fl, ARMS["G5"])[0]
    good_fl = (a0_fl == 8) and (a5_fl == 8)
    notes.append(f"G5 يطابق G0 بعد اعتماد رقمه (إقرار 2026-08-19): "
                 f"G0={a0_fl} · G5={a5_fl} {'✅' if good_fl else '❌'}")
    ok = ok and good_fl
