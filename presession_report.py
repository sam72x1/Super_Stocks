...
# 🗓️ `T-EARLY` (`early_close_prereg.md §②`) — **صفوفُ التدقيق الجديدة** لا تُعيد المنشورَ بالتعريف
#    لو غيّرها تصحيحُ الإغلاق المبكّر ⇒ حارسا `V0`/`V-C0` يوقفان الحكمَ عليها. هذا العلمُ **يطبع**
#    قيمتَيهما بدل التوقّف — ولا تضعه إلّا `early_close_audit.py` **بعد** أن يعبر الحارسان على
#    الصفوف المنشورة بالكود نفسِه في الجوب نفسِه (سلامةُ الأداة مُثبَتةٌ هناك).
#    🔒 مطفأٌ افتراضًا و`== "1"` حرفيًّا ⇒ أداةُ الحكم بت-بت لكلّ تشغيلٍ آخر.
AUDIT_NEW_ROWS = (os.environ.get("PRESESSION_AUDIT_NEW_ROWS") or "").strip() == "1"
...
def main() -> int:
    ...
    if DEV_ON:
        ...
        if (_q0["taken"], _q0["hits"]) != (V0_TAKEN, V0_HITS):
            if AUDIT_NEW_ROWS:
                log("🗓️ **V0 على صفوف التدقيق الجديدة** — تُطبَع ولا توقف "
                    f"(مأخوذ {_q0['taken']} · إصابات {_q0['hits']}) · early_close_prereg §②")
            else:
        ...
        if _shp is not None:
            ...
            # 🗓️ «التدوير» لا يُقال إلّا حين يكون هو الفرق (‏2026-09-24): المشحونةُ
            #    أُعيدت معايرتُها على صفوف `T-EARLY` المصحَّحة (0.69903) ⇒ على الصفوف
            #    المنشورة القديمة يختلف التسليمُ **بإعادة المعايرة لا بالتدوير**.
            _rnd = abs(float(_shp) - float(_thr)) < 1e-5
            log(f"   🔎 وبالأرضية **المشحونة** ({_shp}): مأخوذ {_sh['taken']} · "
                ...
                + ("مطابقٌ للمُعايَر (التدويرُ لم يغيّر تسليمًا)."
                   if (_sh["taken"], _sh["hits"]) == (_q0["taken"], _q0["hits"])
                         ...
                         if _rnd else
                         "مختلفٌ عن المُعايَر — والمشحونةُ مُعايَرةٌ على صفوفٍ "
                         "غيرِ هذي (إعادةُ معايرةٍ لا تدوير · `T-EARLY`).")))
        # ── T-PRECAP
        log("")
    ...
    if CEIL_ON:
        ...
        if _cv != (VC0_HITS, VC0_EXPL):
            if AUDIT_NEW_ROWS:
                log("🗓️ **V-C0 على صفوف التدقيق الجديدة** — تُطبَع ولا توقف "
                    f"(إصابات {_cv[0]} من {_cv[1]}) · early_close_prereg §②")
            else:
