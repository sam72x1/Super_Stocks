...
def fired_of(day: str, path: str = OP_STATE):
    ...
    العدَّ ولا تُبلِغ**. فالموثوقيّةُ أن يكون أحدثُ تاريخٍ في الحالة **مساويًا**
    لليوم المقيس تمامًا:
      • `newest > day`  ⇒ يومٌ تالٍ دهس بعضَ الرسوّ ⇒ **ناقصٌ صامت**
      • `newest < day`  ⇒ العاملُ لم يعمل (أو صفرُ رسوّ) ⇒ لا يُميَّزان
      • `newest == day` ⇒ طازجٌ كاملٌ — وهو حالُ الكرون (‏22:29 UTC، قبل
        بريماركت الغد) ⇒ **المسارُ المجدول لا يتغيّر بهذا الشرط**
    وغيرُ الموثوق `ok=False` واليومُ **يُقصى من `C-LIQ` ويُعَدّ** لا يُطوى (§⑦-4).

    🔴 **والصيغةُ الأولى كانت `newest >= day` فتُمرّر حالةَ الدهس بعينها** —
    أمسكها أوّلُ تشغيلٍ يدويٍّ متأخّر: رسوُّ 2026-09-16 يُقرأ **‏181 ⟶ 61 ⟶ 60**
    عبر ثلاث لقطاتٍ في تاريخ git و`newest` = 2026-09-17 في الثلاث ⇒ الشرطُ
    القديم يقول «موثوق» وهو **ناقصٌ ‏66%**. يُقال ولا يُطوى.

    🔴🔴 **وعطبٌ ثانٍ أثقلُ كُشف 2026-09-17 — والتعريفُ في العقد كان سليمًا
    والكودُ هو الذي خالفه:** §③ يقول «هل **أطلقت** طبقةُ السيولة»، وكانت الصيغةُ
    تعُدّ **كلَّ مفتاحٍ عليه تاريخُ اليوم** — والعاملُ الحيُّ يكتب `date` لكلّ رمزٍ
    **يُقيّمه** (عدّادُ `tc_cum` للزناد الموازي) لا لكلّ رمزٍ **يرسو عليه** ⇒
    `fired` كانت تقرأ «مُقيَّم» لا «أُطلق». والمقيسُ على الحالة الحيّة يوم
    2026-09-17: **‏555 مفتاحًا بتاريخ اليوم · ‏22 منها مِرساةٌ فعليّة** ⇒ من
    قائمتنا الحيّة (‏45) كانت `fired=True` لـ**‏38** والصوابُ **‏1** ⇒ مجتمعُ
    `C-LIQ` (مُكمِّلُ `fired`، **الشاهدُ الحاكمُ الثاني**) ينكمش **‏44 ⟶ 7**
    ⇒ أرضيّتُه (‏50) لا تُبلَغ أبدًا والحكمُ يُدفَع إلى الفرع 3 بصمت.
    ⇒ الشرطُ صار **`anchor_ms` حاضرًا** (المِرساةُ أُنشئت فعلًا).

    🔑 **و`newest` يبقى على كلّ الصفوف المؤرَّخة لا على المراسي وحدَها** —
    عمدًا: صفوفُ التقييم تُحدَّث كلَّ مسحة فتقيس **آخرَ يومٍ عمل فيه العامل**،
    ولو حُسب على المراسي لصار يومٌ بصفر رسوٍّ «بائتًا» وهو طازج."""
    try:
