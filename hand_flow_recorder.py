...
class HandFlowRecorder:
    ...
    def summary(self):
        ...
        if len(dec) >= 20:
            ...
            # 🔴 إصلاح 2026-08-14 (ظهر حيًّا بتشغيلة 31793429251 مرّتين): كان النصّ
            # «(<20)» — و`<20)` تقرؤه تلغرام وسمَ HTML فيسقط الإرسال ("Unsupported
            # start tag") ويُنقذه احتياطُ النصّ الخام فقط. ومخالفٌ أصلًا لقاعدة
            # «بلا علامات مقارنة في أي نص معروض» (2026-06-23). كلماتٌ بدل الرمز.
            lines.append("  عيّنة محسومة قليلة (أقل من 20) — تتراكم حيًّا. لا حكم بعد.")
        return "\n".join(lines)
