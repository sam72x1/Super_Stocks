...
def run() -> int:
    ...
    if R:
        n = len(R["dec"])
        print(f"  ① العيّنة: {n} محسومة (يلزم ≥30 هذي السنة · ≥150 إجمالًا) "
              f"⇒ {'✅' if n >= 30 else '🔴'}")
    ...
    print("\n⚠️ حدود الصدق كما سُجِّلت: NBBO قمّةُ دفترٍ لا عمقه (‏«قابل للتنفيذ» = "
          "وُجد ask معلَن، لا أن الحجم متاحٌ عنده) · انحياز بقاء · تشويه تقسيمات · "
          "الرقم الحرج غير مخزَّن فالمستوى = الأرضية×1.05 (أخفض ⇒ أحداثٌ أكثر لا أقلّ) "
          "· الحسم بلمسة الذيل (محافظ) · ولا وقفَ متحرّكًا ولا خروجًا جزئيًّا (‏T-MANAGE-25).")
    return 0
