...
def _frame_probe(S):
    ...
    **الحدثُ المؤسِّس تقويميّ فمستقلٌّ عن الفريم**، فنأخذ مَن استوفاه
    ...
    ⚠️ **حدُّ صدقٍ في المِجَسّ نفسه:** المضاعف ‏×6 يفترض ستّ شمعات 4س بالجلسة، و
    `fetch_4h` يجلب بـ`prepost=True` فالجلسة الممتدّة ‏≈4 شمعات ⇒ **النافذة على 4س
    أوسع من مكافئها اليوميّ** ⇒ المِجَسّ **متساهلٌ قليلًا مع 4س**. لا يُبطل الفرق
    المرصود (‏16 مقابل 14 = قريب) لكنه يعني أن رقم 4س **سقفٌ لا أرضية**."""
    import os
