...
def main():                                                       # noqa: C901
    ...
    print("  7. والكونُ رقيقٌ (أسهمُ فيصل) ⇒ لا يُعمَّم على كونٍ سائل.")
    print(f"\n⏱️ زمنُ التشغيل: {round(time.time() - t0, 1)}ث")
    # 🔁 **الجدولُ يُعاد آخرًا** — درسٌ مدوَّن في المستودع: «عمليةٌ طويلةٌ
    #    مُخرَجُها يُقتطَع من الآخِر ⇒ أهمُّ سطرٍ يسقط خارج النافذة». قسما ② و③
    #    مسهبان (سببُ كتمٍ لكلّ حدث) فيدفعان الجدولَ خارج `tail` — **ووقع فعلًا
    #    2026-08-18: تعذّرت قراءةُ الجدول من السجلّ** ⇒ يُطبَع مرّتين: أوّلًا وآخرًا.
    print("\n" + "=" * 74)
    print("⑤ 🔁 الجدولُ مُعادًا (أهمُّ سطرٍ يجب أن ينجو من اقتطاع السجلّ)")
    print("=" * 74)
    print("  التوليفة | أسهمٌ تصلك | أحداث | كُتم | 🚩 مراسٍ مكتومة | ⏱️ تأخير")
    for name in PRESETS:
        r = res[name]
        if r["syms"] is None:
            print(f"  {name:<40} | ⚠️ غيرُ مقيس")
            continue
        a = r["anch"]
        _d = "—" if a["delay_med"] is None else f"{a['delay_med']:.1f}د"
        print(f"  {name:<40} | {len(r['syms']):>9} | {r['ev']:>5} | "
              f"{len(r['muted']):>4} | {len(a['muted_anchors']):>13} | {_d:>8}")
