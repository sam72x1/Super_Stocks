...
`V-W6` · `V-W7` · `V-W8`) · 7 الهُويّة (`V-W1`) · 9 «لا قياس» (الفرعُ 3) · **8 المحورُ مُغلَق**.

🔒🔴 **والمحورُ مُغلَقٌ بأمر المالك «اقفل الموجات» (‏2026-09-24 · `waves_result.md §⑨`):** الحكمُ
صدر الفرعَ 2 «لا تُثبَت» موسومًا «سالبة» فالأداةُ تخرج **‏8** قبل أيّ عمليّة، و`WAVES_REOPEN=1`
...
# ═══════════ 🔒 إغلاقُ المحور — «اقفل الموجات» (‏2026-09-24، أمرُ المالك) ══════════
#   الحكمُ صدر **الفرعَ 2 «لا تُثبَت» موسومًا «سالبة»** (‏`waves_result.md`) ⇒ إغلاقٌ
#   **يُنفَّذ لا يُكتَب** (`waves_result.md §⑨` · نمطُ `T-SESSIONS §⑧` حرفيًّا): الأداةُ
#   تخرج ‏8 **قبل** أيّ سطرٍ أو قراءةِ مفتاحٍ أو حارسٍ أو جلب.
AXIS_CLOSED = True
REOPEN_ENV = "WAVES_REOPEN"
...
def _closed_now() -> bool:
    ...
    🔒 **والإقرارُ `1` حرفيًّا** (سابقةُ `SESSIONS_REOPEN`): مُدخَلُ الـworkflow افتراضُه
    `0` ويصل البيئةَ نصًّا غيرَ فارغ ⇒ لو فُحص «غيرُ فارغ» **لارتفع الإغلاقُ في كلّ
    تشغيلة** وهو يبدو مُغلَقًا."""
...
def closure_notice() -> list:
    ...
    return [
        "🔒🔴 **محورُ «موجات السهم بتوقيتنا» (`T-WAVES`) مُغلَق** — بأمر المالك "
        "«اقفل الموجات» (‏2026-09-24) · `waves_result.md §⑨`.",
        "",
        "⚖️ **السبب — بالأرقام لا بالرأي** (`waves_result.md` · التشغيلةُ الحاكمة "
        "‏35902362232 · ‏1,982 سهمَ ماركت):",
        "   • **الفرعُ 2 «لا تُثبَت» موسومًا «سالبة»:** نافذةُ الموجة الثالثة ‏11-12 ET "
        "نالت ‏212 قمّةً = **‏10.70%** وحصّتُها الزمنيّة **‏15.38%** ⇒ **‏0.70×**.",
        "   • **والمعاييرُ الثلاثة ساقطةٌ كلُّها** (`WV1` · `WV2` · `WV3`).",
        "   • **والمتقدّمُ الافتتاحُ عكسَ الدعوى:** ‏09:30-10 نالت ‏27.85% في ‏7.69% "
        "من الوقت = **‏3.62×**.",
        "",
        "🔓 **ولا يُعاد فتحُه إلّا بثلاثةٍ معًا (`waves_result.md §⑨`):**",
        "   ① **مصدرٌ فيصليٌّ جديدٌ للدعوى غيرُ `TG_50819`/`TG_50818`** — وتشغيلُ "
        "الذراع الحاكمة على الدعوى نفسِها ليس مصدرًا.",
        "   ② **تسجيلٌ مسبقٌ جديد** — العقدُ الحاليُّ مدموجٌ لا يُعدَّل، ولا يُرخى فيه "
        "حدٌّ رُئيت قيمتُه (‏`RATIO_MIN` ‏1.5× · `ALPHA_EACH` ‏0.025 · `FLOOR` ‏150 · "
        "`YEAR_MIN` ‏50).",
        "   ③ **إذنُ المالك**.",
        "   ⚖️ والمفتاحُ `WAVES_REOPEN=1` **إقرارٌ بالثلاثة لا التفافٌ عليها** — ويُقرأ `1` حرفيًّا.",
        "",
        "📌 **و`T-SESSIONS` يبقى مُغلَقًا** (`sessions_probe.py` يخرج ‏8) ⇒ **محورُ «ساعة القمّة "
        "داخل الجلسة» مُغلَقٌ للدعويين معًا.**",
    ]
...
def main() -> int:
    # 🔒 الحارسُ **قبل أيّ سطرٍ أو قراءةِ مفتاحٍ أو حارسٍ أو جلب** (‏«اقفل الموجات»).
    if _closed_now():
