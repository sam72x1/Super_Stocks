...
جِد في `test_bot.py` الاختبار:
`"🧪 توصيف: استثناء في كتلة إثراء scan_market يُسقط الفرز كلّه (يُغيَّر في خطة 005)"`
واقلبه: صار المتوقّع أن `scan_market()` **لا ترمي**، وأن السهم ما زال في النتائج.
أزِل تعليق `# ⚠️ خطة 005 ستقلب هذا التوقّع...`.

> لو كانت خطة 001 قد وسمت هذه الخطوة `BLOCKED` (تعذّر بناء التوصيف)، فاكتب الاختبار
> هنا من الصفر بنفس الطريقة الموصوفة في خطة 001 خطوة 6.
