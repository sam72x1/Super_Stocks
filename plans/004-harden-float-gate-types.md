...
### Step 5: اقلب اختبارات التوصيف الخمسة من خطة 001

🔴 **مقاس تجريبيًّا** (طفرة M5 وقت تنفيذ خطة 001 = محاكاة هذا الإصلاح بالضبط):
الإصلاح يُسقط **خمسة** اختبارات، لا اثنين. اقلبها **كلّها** — ولا تحذف أيًّا منها:

| الاختبار في `test_bot.py` | المتوقّع بعد الإصلاح |
|---------------------------|----------------------|
| `🧪 توصيف·M14: فلوت نصّي يرمي TypeError اليوم (يُغيَّر في خطة 004)` | لا يرمي · السهم يبقى · flag «غير متاح» |
| `🧪 توصيف·M14: فلوت NaN يرمي ValueError اليوم (يُغيَّر في خطة 004)` | نفسه |
| `🧪 توصيف·التناقض: refloat_gate_recheck تحرس نفس القيم والبوّابة لا` | **التناقض زال** ⇒ أعِد صياغته: الطبقتان تحرسان الآن |
| `🧪 توصيف·M13: شورت نصّي يرمي TypeError اليوم (يُغيَّر في خطة 004)` | لا يرمي · السهم يبقى · flag «غير متاح» |
| `🧪 توصيف·M13: شورت NaN يرمي ValueError اليوم (يُغيَّر في خطة 004)` | نفسه |

أزِل من فوقها تعليقات `# ⚠️⚠️ خطة 004 ستقلب...` وحدّث الأسماء (احذف «يُغيَّر في خطة 004»).

> لو لم تجد هذه الاختبارات (خطة 001 لم تُنفَّذ) ⇒ **STOP**: نفّذ 001 أولًا.
