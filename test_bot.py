...
_px_cfg = {"MIN_PRICE": 1.649999976158142, "MIN_DROP_FLOOR": 89.58801459243173,
           ...
           "WATCH_MAX_FAILS": 7, "NEAR_PCT": 10.0, "SCORE_MIN": 30,
...
check("🔒 PROX8 بوّاباتُ فيصل **التسع عشرة** كما أخرجتها معايرةُ D11 (شرطُ المالك)",
      all(abs(float(_px_live.get(k, -1)) - float(v)) < 1e-9 for k, v in _px_cfg.items())
      and len(_px_live) == 20, f"طُبِّق {len(_px_live) - 1}")   # 19 + مفتاحُ التفعيل
...
check("🥇 SOFT10 بصمةُ الحوافّ النافذة = معايرةُ D11 (‏`4fb70bcc13e5` · 14 معيارًا)",
      __import__("envelope_scan").edges_fingerprint(
          __import__("envelope_scan").load_edges()) == "4fb70bcc13e5")
