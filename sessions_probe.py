...
🔒🔴 **والمحورُ مُغلَقٌ بأمر المالك «اقفل الجلسات» (‏2026-09-23 ·
`sessions_result.md §⑧`):** الحكمُ صدر الفرعَ 2 «لا ظاهرة» فالأداةُ تخرج
**‏8** قبل أيّ عمليّة، و`SESSIONS_REOPEN=1` **إقرارٌ بشروط الفتح الثلاثة
لا التفافٌ عليها**.
"""
...
# ═══════════ 🔒 إغلاقُ المحور — «اقفل الجلسات» (‏2026-09-23، أمرُ المالك) ══════════
#   الحكمُ صدر **الفرعَ 2 «لا ظاهرة»** والإشارةُ **سالبة** (‏`sessions_result.md`)
#   ⇒ إغلاقٌ **يُنفَّذ لا يُكتَب** (`sessions_result.md §⑧`): الأداةُ تخرج ‏8
#   **قبل** أيّ سطرٍ أو حارسٍ أو نداءِ `git` أو قراءةِ مُدخَل.
AXIS_CLOSED = True
REOPEN_ENV = "SESSIONS_REOPEN"
CLOSED_RC = 8            # مميَّزٌ عمدًا عن 0/3/4/5/**9 «لا حكم»**


def _closed_now() -> bool:
    """أمُغلَقٌ الآن؟ — المفتاحُ يُقرأ **وقت النداء** لا وقت الاستيراد.

    🔒 **والإقرارُ `1` حرفيًّا** (سابقةُ `SUPDEF_REOPEN`) لا «أيُّ قيمةٍ غيرُ
    فارغة»: مُدخَلُ الـworkflow افتراضُه `0` ويصل البيئةَ نصًّا غيرَ فارغ ⇒
    لو فُحص «غيرُ فارغ» **لارتفع الإغلاقُ في كلّ تشغيلة** وهو يبدو مُغلَقًا."""
    return AXIS_CLOSED and (os.environ.get(REOPEN_ENV) or "").strip() != "1"


def closure_notice() -> list:
    """نصُّ الإغلاق — **دالّةٌ نقيّة** ليُقفَل مضمونُها لا شكلُها."""
    return [
        "🔒🔴 **محورُ «ساعةُ القمّة داخلَ الجلسة» (`T-SESSIONS`) مُغلَق** — "
        "بأمر المالك «اقفل الجلسات» (‏2026-09-23) · `sessions_result.md §⑧`.",
        "",
        "⚖️ **السبب — بالأرقام لا بالرأي** (`sessions_result.md` · التشغيلة "
        "‏35500941099 على `main` `d0e4396e` · والضمُّ `V-S2` ‏228 من 228):",
        "   • **الفرعُ 2 «لا ظاهرة»:** دلو الدعوى ‏12-13 ET نال ‏19 من 189 قمّةً "
        "= **‏10.05%** وحصّتُه الزمنيّة **‏15.38%** ⇒ **‏0.65×**.",
        "   • **وشريحتُه من `C-SHUF` ‏28.7** (وسيطُ الصدفة ‏11.38%) ⇒ المرصودُ "
        "**دون الصدفة لا فوقها**.",
        "   • **والمتقدّمُ الصباحُ عكسَ الدعوى:** ‏09:30-12 نال ‏45.50% مقابل "
        "‏38.46% زمنيًّا = **‏1.18×**.",
        "   • **و«يدبل ‏80%» يُكذَّب بمقياسٍ أضعفَ منه عشرَ مرّات:** "
        "`held_and_jumped` (‏+10% لا +100%) = **‏12 من 228 = ‏5.26%**.",
        "   • **والتنبّؤاتُ الخمسة `SP1`-`SP5` مؤكَّدةٌ كلُّها** ⇒ الاتّجاهُ "
        "معروف، والذراعُ الحاكمة (‏≈1,900 يومِ جلب) **مطاردةُ نتيجةٍ معروفة**.",
        "",
        "🔓 **ولا يُعاد فتحُه إلّا بثلاثةٍ معًا (`sessions_result.md §⑧`):**",
        "   ① **مصدرٌ فيصليٌّ جديدٌ للدعوى غيرُ `L316`** — وذاك وسمُه `partial` "
        "(مجتزأ) وجملةُ «‏8:30-9 جلسة لندن» فيه لا تتّسق مع ساعاتها · "
        "**وتشغيلُ الذراع الحاكمة على الدعوى نفسِها ليس مصدرًا**.",
        "   ② **تسجيلٌ مسبقٌ جديد** — العقدُ الحاليُّ مدموجٌ لا يُعدَّل، "
        "**ولا يُرخى فيه حدٌّ رُئيت قيمتُه** (‏1.5× · الشريحة 50 من `C-SHUF`).",
        "   ③ **إذنُ المالك.**",
        "",
...
def main() -> int:                                    # noqa: PLR0911, PLR0912, PLR0915
    # 🔒 الحارسُ **قبل أيّ سطرٍ أو حارسٍ أو نداءِ `git` أو قراءةِ مُدخَل**.
    if _closed_now():
