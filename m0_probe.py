...
def _lock_e1():
    ...
    # 🔴 **بعد أمر المالك «اطفي M0» (2026-08-18):** الفرعُ **غيرُ مشحون** ⇒ لا
    #    معنى لادّعاء «يطابق الإنتاج»، والقياسُ عليه ونشرُ رقمٍ كأنه المشحونُ
    #    **كذبةُ إطار**. ⇒ يُوقَف بسببٍ **مُسمًّى** وبعلاجٍ مكتوب، لا برسالةِ
    #    «تفرّقٌ» غامضةٍ تُقرأ عطبًا في الأداة.
    if not bool(getattr(bot, "LIQ_EARLY", False)):
        return (False, [
            "⛔ `LIQ_EARLY` **مُطفأٌ في الإنتاج** (أمرُ المالك 2026-08-18 بعد "
            "`m0_result.md`) ⇒ فرعُ `M0` غيرُ مشحونٍ فلا يُقاس ويُنشَر كأنه هو.",
            "🛠️ العلاج: إعادةُ شحنه قرارُ مالكٍ صريح — وعندها يعود هذا القفلُ "
            "يقارن بالإنتاج حرفيًّا. (والسويّةُ تُجبِر العلمَ في `MZ2` لتُبقي "
            "الآليّةَ محروسةً وهي مُطفأة.)"])
    ok, notes = True, []
    for name, form, want in cases:
        ...
        notes.append(f"{name}: إنتاج={prod} · مِجَسّ={mine} · متوقَّع={want} "
                     f"{'✅' if same else '❌'}")
        ok = ok and same
    # 🔒 وشاهدٌ يفرّق الذراعَين — **نسبيٌّ لا مغروس (إقرارٌ مؤرَّخ 2026-08-19):**
    #    العيّنةُ القديمة (‏≈$51.6 ألفًا) فرّقت حين كانت الأرضية $30 ألفًا، ولمّا
    #    اعتمد المالكُ $15 ألفًا صارت **فوق ضِعف الأرضية** فمات التفرّق. الآن
    #    تُبنى الدقيقةُ بحيث سيولتُها التراكميّة ‏≈1.4× الأرضيةِ الحيّة — بين
    #    الواحد والضِعفين — فتعبر `E1` وتسقط في `E2` **عند أيّ أرضيةٍ يختارها
    #    المالك** ⇒ الذراعُ ليست زينة.
    _fl = float(bot.LIQ_MIN_USD)
    _vv = int(round((1.4 * _fl - 800.0) / 1.29))
    _f = _b(9, 1.00, 1.30, 1.00, 1.29, _vv)
    e1 = m0_gate(base + [dead], _f, ARMS["E1"]) is not None
    e2 = m0_gate(base + [dead], _f, ARMS["E2"]) is not None
    notes.append(f"الذراعُ `E2` تفرّق (عيّنةٌ نسبيّة ‏≈1.4×الأرضية): "
                 f"E1={e1} · E2={e2} {'✅' if (e1 and not e2) else '❌'}")
