...
def delivered(row: dict) -> bool:
    """§⑤ — «مُسلَّمة» = بلغت المالكَ فعلًا: عمقُ مراحلَ غيرُ فارغ **و**باقيةٌ في آخر
    لقطة (‏`liq_trig_read`: «ظهرت واختفت» = كُتمت ببوّابة المضارب)."""
    return bool((row or {}).get("sent")) and bool((row or {}).get("alive"))
