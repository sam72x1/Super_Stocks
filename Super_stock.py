...
جديد v2.0 — نظام القائمة الأسبوعية الثابتة:
  • السبت صباحًا (على إغلاق الجمعة = شمعة أسبوعية مكتملة): فرز السوق
    كامل → اختيار أفضل 10 أسهم → تثبيتها في weekly_watchlist.json.
  ...
  • السبت: تقرير حصاد الأسبوع (الأداء + أسباب الشطب للتعلم)
...
def borrow_line(r: dict) -> str:
    ...
    صورة 9431): مَن «يشورت» السهم لازم **يستأجره** برسوم سنوية — رسوم عالية + متاح
    قليل = صعب/غالي يشورتونه، وأي انطلاق **يجبرهم يشترون** (يغطّون) فيتسارع الصعود
    = **وقود سكويز**. رسوم شبه صفر + متاح كثير = لا وقود من جهة الاقتراض.
    ...
    if fee >= CONFIG["BORROW_HIGH_PCT"]:
        body = (f"صعب 🔥 (رسوم {fee:.0f}% سنويًّا على الشورت) — وقود سكويز: "
                "أي انطلاق يجبر الشورت يشتري (يغطّي) فيتسارع الصعود")
    elif fee >= CONFIG["BORROW_MID_PCT"]:
        body = f"متوسط (رسوم {fee:.0f}%) — وقود سكويز جزئي"
    else:
        body = (f"سهل ورخيص (رسوم {fee:.0f}%) — "
                "لا وقود سكويز من جهة الاقتراض")
...
def export_weekly_csvs(wl: dict, picks: list, alert_data: dict = None) -> None:
    """يصدّر 3 ملفات CSV للمشرف يوم التجديد (السبت): الصفقات المحسومة · الإشارات الحالية ·
...
WEEKLY_RENEW_DAY = 5                   # 5 = السبت (الاثنين = 0) — التشغيل صباح
                                       # السبت يقرأ إغلاق الجمعة = شمعة أسبوعية
                                       # مكتملة (اثنين→جمعة). كان 4 (الجمعة) لكن
                                       # صباح الجمعة يرى إغلاق الخميس فقط → فريم
                                       # أسبوعي ناقص يوم (طلب المستخدم 2026-07-09).
...
def should_renew(wl: dict, today: dt.date, force: bool = False) -> bool:
    ...
    - غير ذلك: سبت = تجديد (على إغلاق الجمعة) | لا قائمة = تأسيس |
      FORCE_RENEW=1 = إجبار."""
    if force:
        return True
    ...
    # التجديد الكامل = السبت فقط (يقرأ إغلاق الجمعة = الأسبوع مكتمل)؛
    # باقي الأيام = متابعة + إضافة الجديد فقط.
...
def accumulate_explosions(wl: dict, history: dict) -> int:
    ...
    بآخر EXPLOSION_KEEP_DAYS يوم) — يُعرض في تقرير مساعد التطوير (السبت)."""
...
def scan_market():
    """فرز السوق (تجديد السبت أو جلب بدائل) — يرجع (نتائج مرتبة، بيانات)"""
...
def build_daily_message(wl: dict, splits: list,
    ...
    if ready_only:
        lines = [f"📋 <b>قائمة الأسبوع</b> — {today}",
                 ...
                 "(تجديد: السبت على إغلاق الجمعة)", ""]
    else:
        lines = [f"📋 <b>قائمة الأسبوع</b> — {today}",
                 f"{n} سهم: {len(_ready)} جاهز للدخول · {len(_watch)} متابعة "
                 f"(تجديد القائمة: السبت على إغلاق الجمعة)", ""]
    ...
    if replaced and not ready_only:
        ...
        lines.append("(بطاقاتهم الكاملة مع SEC والشورت في تقرير تجديد السبت)")
    if stopped_today:
...
def build_wrapup_message(wl: dict) -> str:
    """حصاد الأسبوع (السبت قبل التجديد، على إغلاق الجمعة): الأداء + أسباب الشطب"""
...
def run_weekly_renewal(wl: dict) -> None:
    """السبت (أو التأسيس الأول): حصاد الأسبوع المنتهي + قائمة جديدة.
    يقرأ إغلاق الجمعة → الشمعة الأسبوعية مكتملة (فريمات M6/الفجوات/الأهداف)."""
    today_iso = dt.date.today().isoformat()
    is_renew_day = dt.date.today().weekday() == WEEKLY_RENEW_DAY
    ...
    title = ("🔄 <b>القائمة الأسبوعية الجديدة</b>" if is_renew_day
             else "🚀 <b>القائمة التأسيسية (تُجدَّد السبت على إغلاق الجمعة)</b>")
    subnote = None
...
def main():
    ...
    if should_renew(wl, today, force):
        why = ("إجبار يدوي" if force else
               ("تجديد نهاية الأسبوع (على إغلاق الجمعة)"
                if today.weekday() == WEEKLY_RENEW_DAY
...
WEEKLY_REPORT_DAY = 5                # 5 = السبت (الاثنين = 0) — مع يوم التجديد:
                                     # تقرير الأداء الأسبوعي يشمل إغلاق الجمعة
                                     # (أسبوع كامل، لا ناقص يوم). غيّره إن أردت.
