...
import traceback
from urllib.parse import quote     # P1-8: إخفاء التوكن بشكله المرمّز-URL أيضًا
...
CONFIG = {
    ...
    "ALGO_PRINT_MAX_SHARES": 10,    # ≤10 سهم = طبعة آلية دقيقة محتملة (N7)
    "ALGO_PRINT_MIN_COUNT": 8,      # أقل عدد طبعات دقيقة خارج NBBO لوسم N7 (صدق العيّنة)
    "QUOTE_FRESH_MS": 5000,         # NBBO أحدث من 5ث = صالح لمقارنة «خارج NBBO» (N7)
...
def hand_evidence(r: dict) -> list:
    ...
    try:
        ...
        # N6 (🆕 درس صور 2026-07-20 «شراء الإغلاق CP»): طبعات كبيرة محايدة التيك (سعر
        # ثابت) = بصمة شراء المضارب على الطبعة الختامية، يعمى عنها كاشف التيك. من نفس
        # flow_raw (فحص اليد فقط). صدق: تأكيد CP الحقيقي يلزمه رموز الشرط غير المتاحة.
        _pr = fr.get("prints") or {}
        _nb = _pr.get("neutral_block_shares") or 0
        if _nb >= CONFIG["OPERATOR_MIN_SHARES"]:
            ev.append({"frame": "طلبات", "sign": "طبعات كبيرة محايدة التيك",
                       "detail": f"~{_nb:,} سهم بسعر ثابت (شراء إغلاق محتمل — يعمى "
                                 "عنها كاشف التيك؛ رموز شرط الإغلاق غير متاحة للتأكيد)"})
        # N7 (🆕 «طبعات آلية»): طبعات دقيقة (≤ALGO_PRINT_MAX_SHARES) خارج NBBO بلقطة
        # طازجة (≤QUOTE_FRESH_MS) = بصمة تجهيز آلي. لا نسِمه على لقطة بائتة (صدق).
        _to = _pr.get("tiny_out_count") or 0
        _age = _pr.get("quote_age_ms")
        if (_to >= CONFIG["ALGO_PRINT_MIN_COUNT"] and _age is not None
                and _age <= CONFIG["QUOTE_FRESH_MS"]):
            ev.append({"frame": "طلبات", "sign": "طبعات آلية دقيقة",
                       "detail": f"{_to} طبعة ≤{CONFIG['ALGO_PRINT_MAX_SHARES']} سهم "
                                 f"خارج NBBO (اقتباس طازج {_age/1000:.0f}ث — بصمة "
                                 "تجهيز آلي؛ لقطة واحدة تقريبي)"})
    except Exception:
        pass
    return ev
...
def _redact_secrets(s) -> str:
    ...
    🔒 **توسعة (مراجعة Codex على 1e322c5):** يغطّي **الشكل المرمّز-URL** (`:`→`%3A`، يظهر
    في أخطاء بعض الوكلاء/المكتبات) ويُطبَّق أيضًا على **جسم رد تيليجرام** (`resp.text` قد
...
def send_telegram_document(path: str, caption: str = "") -> bool:
    ...
    for cid in _admin_chat():
        try:
            ...
            if resp.status_code != 200:
                log(f"⚠️ sendDocument رفض ({resp.status_code}): {_redact_secrets(resp.text)[:160]}")
                ok = False
        except Exception as e:
...
# 🛡️ حجر صحّي للملفات التالفة (①ب · قرار المالك 2026-07-15، مراجعة Codex): ملف حالة
# ثبت تلفه يُسجَّل هنا، و_atomic_write_json يرفض الكتابة عليه بقية العملية ⇒ لا يُكتَب
# الفارغ فوقه، وتبقى آخر نسخة سليمة في سجل git مرجعًا للاسترجاع. (التحذير وحده كان
# يجعل الفقد صاخبًا لا ممنوعًا.) نقطة واحدة: كل الكتّاب يمرّون من _atomic_write_json.
_CORRUPT_STATE_FILES = set()
...
def _handle_corrupt_state_file(path: str, err: Exception) -> None:
    ...
    ⚠️ **ترتيب التعريف حرج (مراجعة Codex على 1e322c5):** `COMPANY_CACHE = _load_company_cache()`
    يُنفَّذ **وقت الاستيراد**؛ لو عُرِّفت هذه الدالّة بعده فكاشُ شركةٍ تالف = `NameError` عند
    الاستيراد = **موت كل workflow قبل أي شيء** (لا تنبيه ولا نسخة ولا فرز). تبقى هنا فوقه."""
    try:
        _CORRUPT_STATE_FILES.add(os.path.abspath(path))   # ①ب حجر: امنع الكتابة فوقه
    except Exception:
        pass
    try:
...
def _atomic_write_json(path: str, data) -> None:
    ...
    🛡️ ①ب حجر صحّي: لا نكتب على ملف ثبت تلفه هذه العملية — تبقى نسخة git السليمة
    مرجعًا للاسترجاع بدل أن يُطمَس بالفارغ (نقطة واحدة يمرّ منها كل الكتّاب)."""
    if os.path.abspath(path) in _CORRUPT_STATE_FILES:
        log(f"🛑 كتابة مرفوضة (ملف محجور تالف): {path} — استرجع النسخة السليمة من git.")
        return
    tmp = f"{path}.tmp"
...
def update_watchlist_status(wl: dict, history: dict) -> list:
    ...
    for s in [x for x in wl["stocks"] if x["status"] == "active"]:
        ...
        # الدورة، يُعاد حين ينجح الجلب) بدل شطب ارتكاز حقيقي أو بنك هدف زائف على بيانات
        # مقسّمة غير مجلوبة. العرض (السعر/الترند/البصمة/التفسير) حُدِّث قبل هذا السطر.
        # ⚠️ أثر سعة عابر مقبول (قرار المالك 2026-07-15): السهم غير المحقِّق يبقى slot_holder
        # حتى ينجح الجلب — أسلم من شطب زائف يفرّغ خانة على بيانات فاسدة · يشفى ذاتيًّا ·
        # لا مسار «عالق للأبد» جديد (سهم بلا بيانات يبقى نشطًا أصلًا بتصميم القائمة الدائمة).
        # 🔒 خارج الفرز/select_top · لا يمسّ _split_scale_factor (قفل C3) · لا LOGIC_VERSION.
        # ⚠️ **إصلاح (مراجعة Codex د3 ثم د3-ب):** الكشف يقتصر على **نافذة التقييم** لا كل
        # التاريخ (~800 شمعة): قفزة تقسيم **قبل** ref_bar مُنعكسة أصلًا في المستويات المخزَّنة
        # (لا تعارض) — فحصها كان يؤجّل زورًا سهمًا نافذته نظيفة وفيها وقف/هدف حقيقي = «عالق للأبد».
        # **شمعة الأساس** = آخر شمعة عند/قبل ref_bar (باعتبارها مرجع المقياس)، تليها شموع ما بعده
        # ⇒ قفزة **إلى** أول شمعة مُقيَّمة تُكشَف حتى لو غاب ref_bar عن الفهرس (added غير تداولي —
        # حالة كل الأسهم النشطة بلا ref_bar). جزء pre-ref واحد فقط فلا يعود التأجيل الزائف.
        _eval_highs, _baseline = [], None
        for i in range(len(df)):
            try:
                ...
                continue
            if _d <= ref_bar:
                _baseline = float(df["High"].iloc[i])   # آخر شمعة عند/قبل المرجع = أساس المقياس
            else:
                _eval_highs.append(float(df["High"].iloc[i]))
        if _baseline is not None:
            _eval_highs = [_baseline] + _eval_highs
        if _raw_splits is None and _split_suspected(_eval_highs):
            log(f"⏳ {s['symbol']}: تعذّر جلب التقسيم + قفزة مقياس مشبوهة (نافذة التقييم) — "
                "تأجيل الحسم (لا نشطب/نحقّق هدفًا على بيانات غير موثوقة).")
            continue
        _spf = _split_scale_factor(_raw_splits, s["added"])
        ...
        # ⚖️ P1-3 (تدقيق Codex 2026-07-14): **الاختلاف عن update_tracking مقصود لا خطأ** —
        # هنا (صيانة القائمة الحيّة) نشطب بالوقف حتى بعد تحقيق هدف («خروج رابح مفترض، النموذج
        # انتهى») لأن القائمة دائمة «تُشطب بالستوب فقط» ويجب ألا تتكدّس. أما update_tracking
        # (سجلّ الأداء) فيتجاهل الوقف بعد الهدف ويسجّله ربحًا (المتداول جنى عند t1). لا نوحّد.
...
def _flow_prints(trades, bid, ask):
    """🆕 ملخّص الطبعات لقرينتَي المضارب N6/N7 (نقيّة · فحص اليد فقط · عرض/تحذير) —
    من دروس صور 2026-07-20. صفقات Polygon [{'price','size'}...] بترتيب **تنازلي**
    (الأحدث أولًا) → نعكسها زمنيًّا لقاعدة التيك. فاشل-آمن → {}:
      • neutral_block_shares (N6 «شراء الإغلاق CP»): مجموع أحجام الطبعات الكبيرة
        (≥OPERATOR_MIN_SHARES) بسعر **مساوٍ لسابقتها** (محايدة التيك) — البصمة التي
        **يعمى عنها** `_operator_blocks` (يصنّف بالتيك فيسقط المحايد بلا اتجاه مؤسَّس).
      • tiny_out_count (N7 «طبعات آلية»): عدد الطبعات الدقيقة (≤ALGO_PRINT_MAX_SHARES)
        المنفَّذة **خارج NBBO** (فوق العرض أو تحت الطلب) = بصمة تجهيز آلي.
      • total. (طزاجة الاقتباس تُحسب في polygon_flow لا هنا — تبقى الدالة نقيّة.)
    ⚠️ صدق: تصنيف تيك تقريبي + قمة الدفتر فقط · تأكيد CP الحقيقي يلزمه رموز شرط
    الطبعة الختامية (Polygon /v3/reference/conditions) غير المتاحة هنا — لا نخمّنها."""
    try:
        rows = [(float(t.get("price")), float(t.get("size")))
                for t in trades if t.get("price") and t.get("size")]
        if len(rows) < 20:
            return {}
        rows = rows[::-1]                             # زمني تصاعدي (كان تنازليًّا)
        neutral = 0
        prev = None
        for p, sz in rows:
            if (prev is not None and abs(p - prev) < 1e-9
                    and sz >= CONFIG["OPERATOR_MIN_SHARES"]):
                neutral += sz
            prev = p
        mn = CONFIG["ALGO_PRINT_MAX_SHARES"]
        tiny_out = 0
...
def polygon_flow(sym: str, with_prints: bool = False):
    ...
    🆕 `with_prints=True` (فحص اليد فقط): يضيف `prints` لقرينتَي N6/N7 من **نفس** صفقات
    الـ100 (صفر نداء إضافي). الافتراضي False = سلوك مسار التنبيه byte-identical."""
    key = os.environ.get("POLYGON_API_KEY", "").strip()
    if not key:
        return None
    try:
        ...
        qt = res.get("t") or res.get("y")            # طابع NBBO (نانو) — لطزاجة N7
        ...
        try:
            ...
            if tr.status_code == 200:
                ...
                if with_prints:                       # 🆕 N6/N7 (فحص اليد فقط)
                    pr = _flow_prints(trades, bid, ask)
                    age = None
                    if qt:
                        try:
                            age = (time.time() * 1e9 - float(qt)) / 1e6
                            if age < 0:
                                age = None
                        except Exception:
                            age = None
...
def build_dev_assistant_report(wl: dict, alert_data: dict = None) -> str:
    ...
    def _pending_verification_block():
        # 🔬 معلّق للتحقّق: إشارات أوّلية تحتاج معايرة/تأكيد **للأمام** (تذكير أسبوعي حتى
        # لا تُنسى — طلب المستخدم 2026-07-23). تذكير عرضي فقط · لا يمسّ الفرز/الحسم.
        return [
            "\n🔬 <b>معلّق للتحقّق (معايرة أوّلية — للأمام)</b>",
            "   🕵️ قروب/مضارب (FSTO + بصمة التّيب: كتل/امتصاص/آيسبرغ/دارك/أكواد): عتبات "
            "أوّلية · لحظي بلا تاريخ → تُعاير على أمثلة فيصل (XHLD قروب · DSY مضارب) أو "
            "قاعدته الحرفية. الباكتيست أثبت أنها ليست بوابة اختيار (القاع هادئ) — القراءة "
            "حيّة بفحص اليد فقط.",
            "   🤖 حصّاد اليد (المرحلة 1→2): ينتظر أسابيع جمع كافٍ + مراجعة الملخّص أعلاه "
            "قبل بناء النموذج.",
            "   📿📏 السلوكيات الثلاثة + حركة فيصل 30-50%: وصفية — تأكيد بصري على أمثلة فيصل.",
            "   📅 أحداث ClinicalTrials.gov + 🌐 ChartExchange (اقتراض/فلوت): يؤكَّد أول "
...
def run_backtest(symbols=None) -> None:
    ...
    market = not symbols
    market_fallback = False        # P1-5: هل سقطنا للكون الاحتياطي (المنحاز)؟
    if market:
        uni = get_universe()
        if uni:
            symbols = uni
            ...
            # وكان التقرير يعنونه «السوق الكامل **بلا انحياز اختيار**» = **عكس الحقيقة**
            # (نسبة نجاح متفائلة تُقرأ كأنها حكم سوق حيادي). الآن يُصرَّح بالانحياز.
            market_fallback = True
            symbols = _default_backtest_symbols()
            log(f"⚠️ باكتيست·السوق: تعذّر جلب ناسداك → كون البوت الاحتياطي "
                f"({len(symbols)} رمز) — **منحاز اختيارًا** (ليس مسح سوق).")
...
def git_save(filenames, runner=None, sender=None):
    ...
    **تلغرام** لا لوقًا فقط. `runner`/`sender` محقونان للاختبار بلا git/شبكة.
    ...
    # 🛡️ توسعة (مراجعة Codex على 1e322c5): الحارس كان يُتجاوَز بحقن المشغّل الحقيقي
    # (`runner=os.system`) — أي اختبار يفعلها يُعيد الحادثة. الآن يُرفض المشغّل الحقيقي
    # نفسه تحت وضع الاختبار (المحاكيات المحقونة تعمل عادي).
    if os.environ.get("SUPER_STOCKS_TESTING") == "1" and (runner is None or runner is os.system):
        return
    try:
...
def update_tracking(data):
    ...
    for a in open_alerts:
        try:
            ...
            # ⚖️ P1-3 (تدقيق Codex 2026-07-14): **الاختلاف عن update_watchlist_status مقصود
            # لا خطأ** — الدالّتان تجيبان سؤالين مختلفين. هنا (سجلّ الأداء): بعد تحقيق هدف
            # (best>0) **يُتجاهَل الوقف** ويُسجَّل hit_tN ربحًا — المتداول يجني عند t1 ويرفع
            # وقفه؛ تسجيله خسارةً لرجوعه للوقف الأصلي يبخس المنهجية. أما update_watchlist_status
            # (صيانة القائمة الحيّة) فيشطب بالوقف بعد الهدف (النموذج انتهى، «تُشطب بالستوب فقط»،
            # كي لا تتكدّس القائمة). توحيدهما يكسر أحدهما — **لا نوحّد، لا تغيير سلوك.**
