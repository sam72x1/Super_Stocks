...
def _mask_id(cid) -> str:
    """يخفي معرّف المستلم في السجل (مراجعة Codex على main-safety، [low] أمني): يُبقي آخر
    4 خانات للتشخيص («…7421») ويخفي الباقي — سجل Actions علني لا يكشف مستلمي التنبيهات."""
    s = str(cid)
    return ("…" + s[-4:]) if len(s) > 4 else "…"


def _redact_secrets(s) -> str:
    """يخفي توكن تيليجرام **ومعرّفات المستلمين** من أي نص يُسجَّل. P1-8 (تدقيق Codex 2026-07-14):
    استثناءات requests (اتصال/TLS/انتهاء المحاولات) وجسم رد تيليجرام تضع الرابط ‏/bot<TOKEN>/…
    في نصّها، وتسجيلها الخام يسرّب التوكن في سجل GitHub Actions العلني.
    🔒 **مستقلّ عن التمثيل (مراجعة Codex على main-safety):**
      • regex على `/bot…/` يخفي التوكن **بأي ترميز** (خام · %3A · %3a · أي هروب) — لا نعتمد
        على مطابقة شكل بعينه (الشكل الكبير كان يمرّ الصغير).
      • + التوكن الحرفي والمرمّز (احتياط لظهوره خارج مسار URL).
      • + كل معرّف مستلم مُهيّأ (`TELEGRAM_CHAT`) يُقنَّع لو انعكس في نص خارجي.
    تسجيل فقط — لا يمسّ الإرسال ولا الفرز. فاشل-آمن → "***"."""
    try:
        s = str(s)
        # (1) أي `/bot<...>/` أو `/bot<...>` بنهاية النص = توكن مهما كان ترميزه.
        s = re.sub(r'(/bot)[^/\s"\']+', r'\1***', s)
        # (2) التوكن الحرفي/المرمّز (خارج URL) — بالحالتين.
        if TELEGRAM_TOKEN:
            _enc = quote(TELEGRAM_TOKEN, safe="")
            for form in (TELEGRAM_TOKEN, _enc, _enc.lower(), _enc.upper()):
                if form and form in s:
                    s = s.replace(form, "***")
        # (3) معرّفات المستلمين المُهيّأة (لو انعكست في جسم رد خارجي).
        for _cid in str(TELEGRAM_CHAT or "").replace(";", ",").split(","):
            _cid = _cid.strip()
        ...
        for cid in recipients:
            try:
                ...
                if resp.status_code != 200:
                    # 🔒 P1-8 (مراجعة Codex 3): **لا نسجّل جسم الرد الحرّ** — تنقيحه سباق تسلّح
                    # لا يُكسَب (ترميزات JSON-unicode/هروب مختلط تمرّ). نكتفي بـHTTP status
                    # (يميّز 400/401/403/429/5xx = الحالات العملية) — صنف التسريب يُغلَق بنيويًّا.
                    log(f"⚠️ تيليجرام رفض ({_mask_id(cid)}): HTTP {resp.status_code}")
                    ok = False
            except Exception as e:
                # 🔒 P1-8: نوع الاستثناء فقط (رسالته قد تضمّ الرابط/التوكن الكامل) — لا نص حرّ.
                log(f"⚠️ خطأ تيليجرام ({_mask_id(cid)}): {type(e).__name__}")
                ok = False
        time.sleep(1)
    return ok
...
def send_telegram_document(path: str, caption: str = "") -> bool:
    ...
    for cid in _admin_chat():
        try:
            ...
            if resp.status_code != 200:
                log(f"⚠️ sendDocument رفض: HTTP {resp.status_code}")   # 🔒 P1-8: بلا جسم حرّ
                ok = False
        except Exception as e:
            log(f"⚠️ خطأ sendDocument: {type(e).__name__}")            # 🔒 P1-8: نوع فقط
            ok = False
    return ok
...
def load_watchlist() -> dict:
    if os.path.exists(WATCH_FILE):
        try:
            ...
            _handle_corrupt_state_file(WATCH_FILE, e)   # P0-5: لا فقد صامت
...
def _collect_closed(wl: dict) -> list:
    ...
    for wk in buckets:
        for s in wk.get("stocks", []):
            ...
            if not (won or lost):
                continue
            # P2-6 (مراجعة Codex على main-safety): المفتاح **بالرمز والسعر فقط** كان يبتلع
            # **ترشيحين مستقلّين** لنفس السهم بنفس entry_ref في أسبوعين (added مختلف) — هنا
            # **قبل** _dedup_closed، فإصلاح المصبّ وحده لا يكفي. أُضيف `added` للمفتاح المنبع.
            key = (s.get("symbol"), s.get("entry_ref"), s.get("added"))
...
def git_save(filenames, runner=None, sender=None):
    ...
    **تلغرام** لا لوقًا فقط. `runner`/`sender` محقونان للاختبار بلا git/شبكة.
    ...
    if runner is None and os.environ.get("SUPER_STOCKS_TESTING") == "1":
        return
    # 🛡️ ①ب (مراجعة Codex على main-safety): استبعد الملفات المحجورة (قُرئت تالفة) — وإلا
    # يرحّلها git_save (git add + إعادة الكتابة عند التعارض) **فوق النسخة السليمة في origin**
    # = تلويث main ببايتات تالفة، تجاوزًا لرفض _atomic_write_json. نقطة الحسم عند الدفع.
    _q = [fn for fn in filenames if os.path.abspath(fn) in _CORRUPT_STATE_FILES]
    if _q:
        log(f"🛑 git_save: تخطّي ملفات محجورة تالفة (لا تُدفَع): {_q} — استرجعها من git أولًا.")
    filenames = [fn for fn in filenames if os.path.abspath(fn) not in _CORRUPT_STATE_FILES]
    if not filenames:
        return
    try:
