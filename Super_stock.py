...
CONFIG = {
    ...
    "WATCH_MAX_FAILS": 3,        # ⚠️ **قيمةُ الرجوع لا القيمةُ النافذة** (خطة 020،
                                 # تصحيح 2026-08-15): مع `FAISAL_ONLY=1` — وهو
                                 # افتراضُ الإنتاج — يُبدِلها ظرفُ الكاتالوج
                                 # (النافذُ اليوم **8**، يُطبَع في رأس كل تشغيلة).
                                 # فلا يُستشهد بالرقم هنا في تشخيصٍ حيّ.
    # 🚪 **البوّاباتُ الصلبةُ الرافضة (هويّةُ الارتكاز):** M1 سعر · M2 هبوط ·
    # M3 انفجار · M4 قاعدة/طزاجة · M5 سيولة — فشلُ أيٍّ منها **رفضٌ فوريّ**.
    # 🔸 **ونواقصُ لينةٌ لا ترفض** (تُحسب ضمن `WATCH_MAX_FAILS`): M6 توافق
    # فريمات · M7 نمط شمعة · M8 فجوة طازجة (لو مفعّلة).
    # ⚠️ تصحيحٌ مؤرَّخ (خطة 020، 2026-08-15): كان مكتوبًا أن الثمانية «إلزامية
    # دائمًا — فشلها = ليس سهم ارتكاز» **ويكذّبه الكود** (‏M6/M7/M8 تُلحَق بـ
    # `soft_fails` ولا تُسقط)، و`analyze_one.render_gate_lines` يسمّيها لينةً
    # صراحةً. وهو صنفُ «توثيقٌ يكذب على كوده» المدوَّن — والتعليقُ هو الخطأ.
    # 📌 و`TF_MIN_REVERSALS` النافذُ **0** بقياس الكاتالوج ⇒ M6 تتحقّق دائمًا.
    ...
    # 🟢 **«لا إشعارَ إلا داخل النطاق»** (قرار المالك 2026-08-06، الخيار ①):
    #    سقفُ ما يُسمَّى «🟢 جاهز للدخول الآن» فوق **أعلى دفعة**، بالنسبة المئوية.
    #    كان التسامحُ الفعليّ **5%** (في `entry_mode`: `price > hi_z * 1.05`) —
    #    والقياسُ الحيّ على القائمة (18 سهمًا) يُظهر أن **9 من 10 «جاهز» فوق
    #    النطاق ويتكدّسون تحت سقف الـ5% بالضبط** (أقصاهم +4.9%) ⇒ التسامحُ هو
    #    الميكانيكا حرفيًّا. والأثرُ المقيس: توقّعٌ **+0.056R ≈ صفر** لأن مسافةَ
    #    المخاطرة تُقاس من الدعم والدخولُ يقع فوق النطاق ⇒ الرابح 0.65R.
    #    ⚖️ و**الصفرُ هو القراءةُ الصحيحة لا التشدّد**: الدفعاتُ أوامرُ حدٍّ **تحت**
    #    السوق، فسعرٌ فوق أعلاها **لا يُعبّئ** ⇒ «ادخل الآن» كانت غير صحيحة، والصحيح
    #    «ضع الطلبات وانتظر النزول». **يُغيَّر بسطرٍ واحد لو أراد المالك تليينه.**
    "ENTRY_READY_BAND_TOL_PCT": 0.0,
    ...
    "SPLIT_CLIFF_PCT": 30.0,             # هبوط يوم حادّ = بصمة كليف التقسيم المعدَّل (خُفّض
                                         ...
                                         #   45 يُسقط 1:1.5 بصمت؛ التقسيم الفعلي يُؤكَّد بالمِجَسّ)
    ...
    # 🔴 **‏20 — قرارُ المالك «ارجع 20» (2026-08-15، بعد قراءة القياس).** نصّان
    #    لفيصل: «تنتظر هبوطه فترة زمنيه · **20 < 30 يوم**» (`IMG_0489`) ·
    #    و«هابط من صعود عالي **من 10 ايام لشهر**» (منشور 2026-08-15).
    #    جُرِّب الأوسعُ (‏10-30) **وقِيست كلفتُه** (`method_window_result.md`:
    #    المطابقُ ‏196 ⟶ 364 = مُضاعَفٌ تقريبًا) ⇒ **فاختار المالكُ الأضيق.**
    #    ⚠️ و«يوم» عندنا **جلسةُ تداول** وقد تكون تقويميّةً عنده — مُعلَنٌ غيرُ محسوم.
    "METHOD_DECLINE_MIN": 20,            # «‏20 < 30 يوم» — قرارُ المالك 2026-08-15
    "METHOD_DECLINE_MAX": 30,            # «لشهر» · وهو الحدّ الأعلى في النصّين
    # 🔴 **شرطُ `RSI` أُلغي بأمر المالك «الغ شرط rsi» (2026-08-15) بعد القياس.**
    #    نصُّه «‏+ RSI تحت 30» طُبِّق بوّابةَ رفضٍ ساعاتٍ ثم أُلغي، والقياسُ سببُه:
    #    **يقصّ 89.8%** (‏196 ⟶ 20) **ويُخرج أسماءً من كتالوج فيصل نفسِه**
    #    (‏2024: `AUUD` و`ZCMD` · 2025: `HUBC`) ⇒ قرارُ مالكٍ صريح.
    #    و`RSI` **يبقى عرضًا** على الكرت بنطاق «من 22 لـ27» (`FAISAL_RSI_LO/HI`)
    #    — الحقلُ يُحسب ويُخزَّن، **ولا يرفض أحدًا**.
    ...
    #    محلَّ أرقامنا: **‏16 تعيينًا** (‏11 معيارًا ⟹ 12 مفتاحًا صلبًا لأن `drop_pct`
    #    مفتاحان + 4 عتبات «مثاليّ» من الوسيط). `FAISAL_ONLY=0` يرجع لبوّابات البوت.
    #    🔴 **تصحيحٌ مقيس (2026-08-08):** كان مكتوبًا هنا أن ثلاثًا «تبقى برقمنا»
    #    (`RECENT_RISE_BLOCK_PCT`=35 · `MA_GATE_MAX_ABOVE_PCT`=15 ·
    #    `GAP_ABOVE_MAX_DIST_PCT`=60) — **وهو خطأ: الثلاثةُ مُستبدَلةٌ فعلًا**.
    #    قِيس بطباعة `CONFIG` بعد التطبيق: ‏214.73 · 95.47 · 1134.78 (وسجلُّ الفرز
    #    يقول «طُبِّق 20 رقمًا» ويسمّيها). وسببُ الخطأ أنّي قرأتُ **الافتراض** في
    #    هذا الجدول (‏35 أعلاه) لا **النافذ** بعد `faisal_only_overrides` ⇒ نفسُ درس
    #    «اقرأ القيمة النافذة لا المكتوبة». **وما لا يقيسه الظرف حقًّا**:
    #    `TF_MIN_REVERSALS` (مُصرَّح به) + `M13`/`M14` + الثوابت البنيوية.
    #    ⚠️ و`RECENT_RISE_BLOCK_PCT`=214.73 يعني **بوّابةَ منع الملاحقة (D11)
    #    معطَّلةً عمليًّا** — ومعها سطرُ «انفجر خلال أسبوع» في المتابعة (‏:10680).
    #    السببُ بنيويّ: `P100` = **أقصى** نافذةٍ في الكاتالوج، ووسيطُه **‏−8.26%**
    #    ⇒ نافذةٌ واحدة من 500 تضبط البوّابة. **تغييرُه قرارُ مالك** (يمسّ العضوية).
...
# 🥇🔴 **قرارُ المالك 2026-08-10 «اعتمد 120» — سقفُ عرض القاعدة (‏M4) يعلو الظرف.**
# سندُه `base2_result.md` (‏`T-BASE-2`، تسجيلٌ مسبق `62865c0` **قبل أيّ رقم**): ذراعُ
# `B120` استوفت الحرّاسَ الأربعة — المنفجرون المُسلَّمون (`d50`) المجمَّع **‏53 مقابل
# 53 = صفرُ كلفةٍ مقيسة** · أقصى نقصٍ سنويّ **‏18.2%** (الحدّ 20) · `R`/صفقة يتحسّن
# ‏+0.058 · **وترفض `LGHL`** (عرضُ قاعدته المقيس ‏160.8%) · وبـ**‏8,756** رفضةً مقابل
# **‏46,712** لـ`B40` (‏5.3× أرخص) بينما `B40` يُكلّف **‏22.6%** من التسليم.
# ⚠️ **والتحفّظُ منشورٌ مع القرار لا مكتومًا** (`base2_result.md §③`): الإشارةُ
# **تنقلب** (‏+25.0% · −18.2% · −9.1%) والتساوي تصنعه 2024 وحدها، فالمقيسُ **«لم
# يُكلّف» لا «ربّح»** — وهذي **تجربةُ كلفة**، ومعيارُها عدمُ التدهور لا التحسّن.
#
# 🥇 **ووسمُه `engineering` لا `faisal_*` — مُعلَنٌ صريحًا:** ‏120 **رقمُ قرارٍ** لا
# رقمَ كاتالوج (أرقامُ الكاتالوج للقاعدة: **‏475.2** بـ`P90` و**‏23,684.63** بـ`P100`
# النافذ) ⇒ فهو **استثناءٌ صريح** من «أرقامُ فيصل وحدها»، مدوَّنٌ في
# `FAISAL_SOURCE_LEDGER.md` بوسمه.
#
# 🔒 **ولماذا طبقةٌ *بعد* الظرف لا تعديلَ ملفِّ الحوافّ:** إعادةُ معايرة الظرف
# (‏`catalog_envelope.yml`) تُصدر `base_range` جديدًا **فتدهس القرار صامتةً** — وهو
# صنفُ «الانقلاب الصامت» بعينه. فتُطبَّق **آخرًا** فلا يُزيلها إصدارُ ظرفٍ جديد،
# **وتُطبَع باسمها** فيبقى الفارزُ مقروءًا لا صندوقًا أسود.
# ⚠️ **ولا تُطبَّق عند `FAISAL_ONLY=0`** (عمدًا): بوّابةُ البوت هناك **‏40** وهي
# **أشدُّ** من 120، ورفعُها كان **إرخاءَ** بوّابةٍ لم يطلب المالكُ إرخاءها (وقفلا
# `B3`/`C3` يحرسان الأربعين بحكمٍ مقيس).
OWNER_GATE_OVERRIDES = {"BASE_RANGE_MAX_PCT": 120.0}
...
def apply_faisal_only(cfg: dict = None, log_fn=None) -> dict:
    ...
    ov.update(_soft)
    # 🥇 و**قرارُ المالك يعلو الظرفَ — ويُطبَّق آخرًا حصرًا** (‏§`OWNER_GATE_OVERRIDES`)
    #    فلا يدهسه إصدارُ حوافَّ جديد. **والترويسةُ تُصرّح بعددِه** وإلّا صار رقمُ قرارٍ
    #    مدفونًا في قائمةٍ عنوانُها «معايير فيصل وحدها» = سطرُ عرضٍ يكذب.
    ov.update(OWNER_GATE_OVERRIDES)
    cfg.update(ov)
    _own = len(OWNER_GATE_OVERRIDES)
    (log_fn or print)(
        "🥇 **معايير فيصل وحدها** مُفعَّلة (أمر المالك) · بصمةُ الحوافّ "
        f"{fp[:12]} · طُبِّق {len(ov)} رقمًا"
        + (f" (منها **{_own}** بقرار المالك لا من الكاتالوج)" if _own else "")
    ...
    if _own:
        (log_fn or print)(
            "   ↳ 🥇 **بقرار المالك** (وسمُ `engineering` — لا من الكاتالوج · "
            "سندُه `T-BASE-2`): "
            + " · ".join(f"{k}={v}"
                         for k, v in sorted(OWNER_GATE_OVERRIDES.items())))
    return ov
...
def _split_day_value(closes, splits, cut):
    """🎯 **قيمة شمعة يوم التقسيم** — مرجع فيصل الحرفي للـ÷2 (IMG_0150: «اقسم قيمة الشمعة
    ÷2»؛ WORX 2.33÷2=1.16). = إغلاق **أول شمعة بعد آخر تقسيم عكسي** (نسبة<1، ≤cut)، لا
    أعلى قمة متأخّرة. **تصحيح 2026-07-24 (غلط تطبيقنا):** كان الصيّاد يستعمل `_post_split_high`
    (ذروة الرفعة بعد التقسيم على High بذيولها) — فيخطئ الأسهم التي رفعها المضارب بعد التقسيم.
    فيصل يقسم شمعة يوم التقسيم نفسها. None لو لا تقسيم عكسي حديث. نقيّة · فاشلة-آمنة · بلا تسريب."""
    try:
        if closes is None or splits is None:
            return None
        if hasattr(splits, "__len__") and len(splits) == 0:
            return None
        ...
        if not rsplits:
            return None
        last = max(rsplits)
        post = sorted(((_n(ts), float(v)) for ts, v in closes.items()
                       if _n(ts) >= last), key=lambda x: x[0])
...
def half_down_target(df, lookback: int = 60, hold: int = 3, price=None,
    ...
    """🪦 **متقاعدة من العرض (2026-07-27، مراجعة خصومية) — محفوظة كبنية بحث فقط.**
    السبب: تعريفنا الإجرائي «أدنى إغلاق في 60 جلسة مضى عليه 3 جلسات» هو **وصف حرفي
    لقاعدة كل سهم ارتكاز** (ثبات 3-8 جلسات على الدعم)، فكان السطر يظهر على **كل**
    مرشّح: «مستهدف هبوط −50%» بجوار دفعات الشراء ووقف 7% وسحب سيولة 7-13% = ثلاثة
    تقديرات متناقضة على البطاقة نفسها. وبطاقة MWC (السند الوحيد) لا تذكر «60 جلسة»
    ولا «أدنى إغلاق» — فالتشغيل اجتهادٌ مُلبَس ثوب قاعدة. **لا تُوصَل بأي عرض قبل
    تعريف «المستوى السائد» من نصّ فيصل نفسه + موافقة المستخدم.**
...
_SEC_FOUNDING = {}               # النشرة النهائية فقط = الحدث المؤسِّس
...
def sec_recent_filings(sym: str):
    ...
    try:
        ...
        for i in range(len(forms)):
            ...
            # ⚠️ **قناتان منفصلتان (تدقيق خصومي):** الواسعة للسياق فقط، و**المؤسِّسة**
            # للنشرة النهائية حصرًا. بقناة واحدة كان أحدثُ `S-3` روتيني **يحجب**
            # النشرة الحقيقية الأقدم فتموت الميزة، **ويُعرَض التسجيل الرفّي نفسه**
            # كأنه «طرح جديد — حدث مؤسِّس».
            ...
            if (form in _FOUNDING_OFFERING_FORMS
...
def _parse_form4(xml: str):
    ...
    ⚠️ **حدّان يُصرَّح بهما (تدقيق خصومي):** (1) الرمز **A** وحده لا يكفي (المنح `A`
    والممارسة `M` تظهر «مكتسبة» بلا شراء نقدي) فنشترط `P` صراحةً. **هذا اختيارنا
    المحافظ لا نقلٌ عن فيصل** — فيصل لم يذكر رموز SEC قط؛ كلامه «اشترى الرئيس 12 ألف
    سهم بسعر 1.37» يوافق `P`، أما «المدير اشترى 1.7 مليون سهم» فقد يكون صفقة خاصة أو
    ممارسة أدوات تُبلَّغ برمز آخر أو في الجدول المشتقّ **الذي لا نقرأه**. (2) نقرأ
    `<nonDerivativeTransaction>` فقط. فالفراغ = «لم نجد شراءً ضمن ما فحصناه» لا نفي.
    ...
    **لا يُنادى في مسار الفرز** (enrich بعد select_top فقط). عرض/سياق — لا بوّابة."""
    ...
    for m in meta[:max(0, int(cap))]:
        ...
        if not acc or not doc:
            continue
        # ⚠️ **إصلاح حاسم (تدقيق خصومي، مُثبَت على 614/614 إيداع حقيقي):** حقل
        # `primaryDocument` لنماذج الملكية يأتي **بسابقة عرض XSL** (`xslF345X03/…`)
        # وذلك المسار يعيد **HTML مُصيَّرًا** بلا وسوم XML ⇒ المحلّل يرجع None دائمًا
        # فتكون الميزة ميتة **وتستهلك الميزانية كاملةً صامتةً** (الردّ 200 فلا يُحسب
        # إخفاقًا ولا يقطع الدائرة). الملف الخام = الاسم نفسه بلا السابقة.
        url = f"https://www.sec.gov/Archives/edgar/data/{cik}/{acc}/{doc.split('/')[-1]}"
        try:
        ...
        if p:
            p["date"] = m.get("date")
            out.append(p)
        elif "<transactionCode>" not in txt:
            # مستند وصل لكنه ليس XML الملكية (صفحة مُصيَّرة/خطأ) — إخفاق حقيقي يجب
            # أن يراه قاطع الدائرة، وإلا مات المسار بصمت كما كشف التدقيق.
            _F4_FAILS[0] += 1
    return out
...
def _latest_event_date(r, today=None, max_age_days: int = 21):
    ...
    try:
        ...
        # (دعوة الجمعية `proxy_filing` **ليست خبرًا** يُقبَل أو يُرفَض — أُسقطت)
        for k in ("offering_event",):
            ev = (r or {}).get(k) or {}
...
def news_acceptance(df, event_date):
    ...
    ⚠️ **المرجع من شمعة الحدث نفسها حصرًا** (تصحيح مراجعة خصومية 2026-07-27): كانت
    تقارن إغلاق يوم الحدث بـ**الرقم الحرج المحسوب اليوم** — فارق زمني يقلب الحكم بلا
    حدث جديد، وأسوأ: الرقم الحرج بنيويًّا أعلى من سعر اليوم، فـ«مقبول» ما كان ممكنًا
    إلا لسهم **هبط** ⇒ التحذير الهابط كان يشتعل لمن **ارتفع** (إشارة مقلوبة).
    الآن: القبول = إغلاق شمعة الحدث **فوق افتتاحها وفوق إغلاق الجلسة السابقة** — حكم
    مكتفٍ بذاته، ثابت زمنيًّا، ويقرأ حرفيًّا «هل قَبِل السوقُ الإعلانَ يومَه».
    ...
    try:
        ...
        op = float(df["Open"].iloc[hit])
        prev = float(df["Close"].iloc[hit - 1]) if hit > 0 else op
        ref = max(op, prev)
        kind = "افتتاحه وإغلاق ما قبله"
        if not (ref == ref) or ref <= 0:        # NaN/تالف ⇒ لا حكم
            return None
...
def news_rejected_line(na) -> str:
    ...
    if not isinstance(na, dict) or na.get("accepted") is not False:
        return ""
    return (f"⚠️ إعلان {na.get('date', '')} لم يُقبَل: أغلق ${na['close']:g} تحت "
    ...
    for r in results:
        ...
        try:
            ...
            try:
                ...
                # (نظيرُ قاعدة «تعذّر ≠ صفر» مقلوبةً). الآن: الصفرُ يُعرَض صفرًا،
                # و`None` وحدَه يرتدّ للذاكرة.
                r["short_pct"] = (round(sp * 100, 1) if sp is not None
...
def _offering_event(sym, today=None, fetch=None, lookback=None):
    ...
    try:
        ...
        for i, f in enumerate(forms):
            # **امسح كل القائمة** ولا ترجع عند أول تطابق واسع: النماذج مرتّبة من
            # الأحدث، وأحدثُ تسجيل رفّي روتيني كان يحجب النشرة النهائية الأقدم.
            if (f or "").strip() in _FOUNDING_OFFERING_FORMS:
                ev = _fresh({"form": (f or "").strip(),
...
def scan_split_radar(history, exclude=None, fetch_splits=None, fetch_borrow=None,
    ...
    <20ألف · حافظت 3 جلسات · خالية من القروبات · هدف 100%+) **و«لم تصعد أكثر من 20%»
    بعد الحدث** (‏`IMG_0153` — أُضيفت بأمر المالك 2026-08-13 «صلّح الرادار» بعد أن كُشف
    أن الرادار كان يتخطّاها وحدَه فيعرض ما يرفضه الصيّاد ⇒ الآن **ستّةٌ لا خمس**).
    لا يمسّ الاختيار/الدخول/
    ...
    **🕵️ الشورت = عمود Available من ChartExchange** (`ce_borrow_info` — قراءة فيصل الموثّقة
    ...
    for p in probed:
        ...
        # 🔴 **صارت 0-6 بأمر المالك 2026-08-13:** أُضيف `didnt_rise` (نصُّ فيصل) بعد
        #    أن كان الرادارُ يتخطّاه. ⇒ **جاهز ⟺ match == 6**، والرتبةُ محفوظة:
        #    الجاهزُ يبقى في القمّة فلا يقصّه `SPLIT_RADAR_MAX` (كان جاهز ⟺ 5).
...
def split_radar_ready(rows: list) -> list:
    ...
    🔴 **تصحيحٌ بأمر المالك 2026-08-13 («صلّح الرادار») — العيبُ الذي أُصلح:** كان
    الجاهزُ **خمسًا من ستّ**: قرب القاع÷2 · حافظ 3ج · شورت (متاح CE) · فلوت<2م ·
    خالٍ من قروب — **و«لم يصعد أكثر من 20%» غائبةٌ كليًّا** مع أنها **نصُّ فيصل
    حرفيًّا** (`IMG_0153`: «شرط ما تجاوز 20٪ اثناء الصعود» · وسمُ الدفتر
    `faisal_verbatim`) وتُحسب في `_split_setup_probe` وتُشترَط في `scan_split_hunter`.
    ⇒ فكانت الترويسةُ «المطابق الكامل» **تَعِد بأكثر مما تفحص** = عينُ عيب «كرت
    البوّابات» الذي أمسكه المالك (إطارٌ يكذب على قارئه). **وأثرُه مقيسٌ حيًّا:**
    ليلةَ 2026-08-12 عرض الرادارُ `LZMH` و`CHSN` **والصيّادُ صامت** — والفرقُ
    البنيويُّ الوحيدُ بينهما هذي الشرط.
    ⚖️ **والمقايضةُ كانت مقلوبة:** يرفض على **المتاح للاقتراض** (متغيّرٌ يتحرّك خمسَ
    مراتب يوميًّا بالمقيس) ويتساهل في **شرط فيصل الثابت** — والصيّادُ يفعل العكس.

    ⇒ الجاهز الآن = **الستّة كلها**. دالّة نقيّة، لا تُسقط معيارًا ولا تخترع واحدًا —
    نفس الستّة التي تُحسب في `scan_split_radar` (`match` صار 0-6 ⇒ **جاهز ⟺ 6**).
    🔒 و`didnt_rise` تُقرأ بنفس التحقّق البسيط كالخمسة (اتّساقُ نمط)، **ومفتاحُها
    مضمونُ الحضور** من `_split_setup_probe` دائمًا (فاشلةٌ-آمنة داخلها: بلا قيمةِ
    شمعةٍ ⇒ `True`) ⇒ **يستحيل أن تقصّ مرشّحًا حقيقيًّا بصمت** — مقفولٌ `RDR6ج`."""
    return [r for r in (rows or [])
            if r.get("near_bottom") and r.get("held_ok") and r.get("short_ok")
            and r.get("float_ok") and not r.get("pump")
            and r.get("didnt_rise")]      # ④ نصُّ فيصل — كانت غائبةً حتى 2026-08-13
...
def build_split_radar_section(rows: list) -> str:
    ...
    **لم يصعد**).
    ...
    ✅ **و«الكامل» صار صادقًا فعلًا (2026-08-13، أمرُ المالك «صلّح الرادار»):** كان
    يفحص **خمسًا من ستّ** ويُسمّي نفسَه «الكامل» — فأُضيف شرطُ فيصل ④ إلى
    `split_radar_ready`، **ويُطبَع صراحةً برقمه** في سطرٍ مستقلّ فلا تبقى الترويسةُ
    دعوًى بلا شاهد (درسُ «سطرُ عرضٍ بلا حقلٍ = كذبة» معكوسًا: **شرطٌ بلا سطرٍ = دعوًى
    غيرُ قابلةٍ للفحص**)."""
    ...
    for r in rows:
        ...
        # ④ نصُّ فيصل «شرط ما تجاوز 20٪ اثناء الصعود» — **يُطبَع برقمه** فلا تبقى
        #   ترويسةُ «المطابق الكامل» دعوًى بلا شاهد (أُضيف 2026-08-13 مع الشرط نفسه).
        _rp = r.get("rose_pct")
        lines.append(f"  {chk(r.get('didnt_rise'))} لم يصعد بعد الحدث "
                     + (f"{_rp:.0f}%" if isinstance(_rp, (int, float))
                        else "—")
                     + f" (شرط فيصل: لا يتجاوز "
                       f"{CONFIG['SPLIT_ROSE_MAX_PCT']:.0f}%)")
...
_METHOD_FOUNDING = []
# 🩺 عدّاداتُ آخر مسح — تُنشَر ليقرأها المِجَسّ والتشخيص **بالرقم الحقيقيّ**.
# 🐞 وسببُها عيبٌ في كودي: كان المِجَسّ يقارن 4س بـ`len(_METHOD_NEAR)` وهي
#    **عددُ القريبين لا عددُ مَن بلغ التسلسل** — يتساويان **فقط** حين لا مطابق
#    كامل. فأوّلُ يومٍ يُنتج مطابقًا كانت المقارنة ستُطبَع **مبخوسة بصمت**.
_METHOD_STAGE = {}
...
# رمزًا) ⇒ سقفُ 10 كان **أضيق من العدد المعتاد** فيقصّ كلَّ يومٍ تقريبًا. و25
# يُظهر اليومَ المعتاد كاملًا ويبقى صمّامًا لليوم الشاذّ (مع إعلان العدد المقصوص).
METHOD_NEAR_SHOW = 25
    ...
    **الشروط الستّة (تُشترط كلُّها — نصّ فيصل حرفيًّا):**
    ...
    for sym, df in (history or {}).items():
        try:
            ...
            if price > entry:
                # 🔴 **النسبة في نصّ السبب** (سؤال المالك 2026-08-01): الترتيب صار
                #    بالأقرب إلى الدخول، **وترتيبٌ لا يُرى سببُه يبدو عشوائيًّا**
                #    — فيُقرأ اختفاءُ اسمٍ من العرض «حذفًا» لا «إبعادًا».
                _near(f"تجاوز منطقة الدخول بـ{(price / entry - 1.0) * 100:.1f}% "
                      f"(الدخول ${entry:.2f})")
                continue
            stage["entry_zone"] += 1
            ...
            # 📐 **`RSI` عرضًا لا بوّابة** — أُلغيت البوّابةُ بأمر المالك «الغ شرط
            #    rsi» (2026-08-15) بعد أن أراه القياسُ أنها تقصّ **‏89.8%** وتُخرج
            #    أسماءً من كتالوج فيصل نفسِه. **فلا `continue` هنا ولا رفض**؛
            #    الحقلُ يُحسب ليحيا سطرُ «‏📐 RSI … (فيصل: من 22 لـ27)» في الكرت.
            # 🔒 ويُميَّز المحسوبُ من المحشوّ لأن `rsi()` تنتهي بـ`fillna(50)`:
            #    رقمٌ محشوٌّ يُعرَض على أنه قراءةٌ **كذبةُ عرضٍ** وإن لم يرفض أحدًا.
            _rsi = None
            try:
                _cl = df["Close"].astype(float)
                if len(_cl) >= 20 and bool((_cl.diff().tail(42) < 0).any()):
                    _v = float(rsi(_cl).iloc[-1])
                    _rsi = _v if _v == _v else None     # NaN ⇒ لا يُعرَض
            except Exception:                                    # noqa: BLE001
                _rsi = None
            try:
    ...
    #    الزخم · **يوميّ/أسبوعيّ لأسهم التجميع** · 4 ساعات **بعد** تكوين قاع أو ارتداد»
    #    — وفئتُنا التجميع ⇒ الكبيرُ يؤهّل والصغيرُ يأتي بعده · ويؤكّده `IMG_0080`
    #    «شمعة 4س هابطة ⟸ **انتظر اليوميَّ يختبرها**» (اليوميُّ حاكم).
    # ⇒ المفتاحُ الأوّل **الفريم** (اليوميّ قبل 4س) ثم الصعود. والقراءةُ على الفريمين
    #    كانت مبنيّةً سلفًا (اليوميّ أوّلًا و4س لمن سقط عليه) والفريمُ مُسمّى في الكرت —
    #    الناقصُ كان **الترتيب** وحده.
    # ⚠️ **حدُّ صدق:** الترتيبُ يسبق قصَّ `METHOD_MAX` (=8) ⇒ عند تجاوز الثمانية قد
    #    يبدّل **مَن يصل**، وهو المقصود بالقاعدة (الكبيرُ أولى) لا أثرٌ جانبيّ.
    rows.sort(key=lambda r: (0 if str(r.get("frame") or "") == "يوميّ" else 1,
                             -(r["t1"] / max(r["entry"], 1e-9))))
...
def build_method_alert(rows: list, today=None) -> str:
    ...
    if _nl:
        lines.extend(_nl + [""])
    # 🐞 **حدُّ صدقٍ بائتٌ صار كذبًا (2026-08-01):** كان السطر يقول «العيّنة 27 دون
    #    30» محيلًا إلى `T-METHOD` — **وتلك التجربة قاست مجتمعَ المسح لا مجتمعَ فيصل**
    #    (المرحلة ③ كانت معكوسة) فسُحبت دلالتُها. ⇒ الصادق أن الوصفة **بلا حكمٍ
    #    تاريخيّ أصلًا**، لا أنها «حُكم عليها بعيّنةٍ صغيرة».
    lines.append("<i>⚠️ وصفةٌ <b>قيد الإثبات الأماميّ</b> — لم تُختبَر تاريخيًّا "
                 "بعد (تجربة T-METHOD قاست قراءةً مختلفة فسُحبت دلالتُها). "
                 "تُقرأ بحذر.</i>")
    return _rtl_join(lines)
...
def reversal_candle(ohlc):
    ...
    if (c2 < o2 and c3 > o3 and max(o3, c3) <= o2 and min(o3, c3) >= c2):
        return "هرامي"
    # ④ «وت» (ابتلاع صاعد) — 📖 **تصحيحُ إسنادٍ مؤرَّخ (2026-08-15، منشور X
    #    أرسله المالك):** نصُّ فيصل يقرأ «همر > نجمة صباح > **هرامي وت ايفر**»
    #    أي "whatever" = **«أيًّا كانت»** (مطابقًا لجملته قبلها «ايا كانت نوع
    #    هذي الشمعه») — فالمرتبةُ الرابعة عنده **بابٌ مفتوح لأيّ شمعةٍ انعكاسية**
    #    لا نموذجٌ سمّاه «وت». تنفيذُنا (الابتلاعُ الصاعد — أشهرُ الأشكال
    #    القياسية الباقية) **مثالٌ صحيحٌ من «أيًّا كانت»** فيبقى، والاسمُ «وت»
    #    اصطلاحُنا الداخليّ للمرتبة لا اسمَ فيصل. صفرُ تغيير سلوك.
...
def build_split_hunter_alert(rows: list, today=None, fetch_hist=None,
    ...
    for r in rows:
        ...
        for _x in hunter_extras(r, df=r.get("_df"), flow=r.get("_flow"),
                                df4h=r.get("_df4h")):
            lines.append(_x)
        # ✅ **قواعدُ «النماذج التعليمية» — أُضيفت بإذن المالك (2026-08-06).**
        #    كانت موقوفةً منذ 08-05 لأن **درعَ الصيّاد** أسقطها، وقاعدةُ المالك
        #    «صيّاد المقسّم استثناءٌ … **اتركها على جنب** وأبلغني». أُبلِغ فأذِن
        #    بشرطٍ حرفيّ: «**سوها لو ما تاثر على الفحص و الفرز نفسه**».
        # 🔒 والشرطُ مستوفًى **بنيويًّا لا بالوعد**: النداءُ هنا في **بانيَ الرسالة**
        #    بعد اكتمال الكرت، و`scan_split_hunter` (الفحصُ والفرز) **byte-identical**
        #    ولا تعرف هذي الدوالّ أصلًا · والحقولُ تصل مُثراةً من `split_hunter.py`
        #    **بعد** الاختيار وبعد حارس الافتر ⇒ يستحيل أن تُدخل مرشّحًا أو تُخرجه.
        #    مقفولٌ باختبارٍ سلوكيّ: نفسُ المطابقين مجموعةً وترتيبًا مع الإثراء وبدونه.
        # ⚠️ **المفاتيحُ مأخوذةٌ من مُخرَج `scan_split_hunter` الفعليّ** (`price` ·
        #    `avail`) و`_rsi` يُحسب في الإثراء. وأوّلُ توصيلٍ كتبتُه استعمل
        #    `shares_available`/`rsi`/`entry_low` — **وثلاثتُها لا وجود لها في الصفّ**
        #    ⇒ كانت ستكون **أسطرًا ميّتة** لا يكشفها اختبارٌ أخضر. (درسُ «نقطة النداء».)
        # 📌 و`entry` **لا يُمرَّر عمدًا**: لا مفتاحَ دخولٍ صريحًا في الصفّ، واختراعُ
        #    واحدٍ مخالفٌ لأمر المالك «لا تجيب شي من عندك» ⇒ يُسقَط سطرُ «الهدف الأوّل»
        #    بصمتٍ كما تفعل الدالّة مع كلّ غائب.
        for _x in faisal_rule_lines(r.get("_df"), price=r.get("price"),
                                    avail=r.get("avail"),
                                    rsi_val=r.get("_rsi")):
            lines.append(_x)
        lines.append("")
    ...
    try:
        ...
        def _target_source(tv):
            ...
            if _lib and abs(t2 - round(float(_lib), 2)) < 0.005:
                return "مستوى التحرر"
            # 🔑 خطة 019-أ: المفتاحُ في `all_unfilled_gaps_above` اسمُه
            # **`all_zones`** (‏:2760) لا `zones` — فالقراءةُ القديمة كانت تُرجع
            # `None` دائمًا ⇒ وسمُ «حافة فجوة فوقية» **لم يظهر قطّ**. (والمفردةُ
            # `unfilled_gaps_above` مفتاحُها `zones` فعلًا — لا تُخلَط بها.)
...
def _market_holidays() -> set:
    ...
    ⚠️ **وحدٌّ مُعلَن:** `market_calendar` مُغطًّى حتى **2027** (خطة 014، 2026-08-15 —
    كان 2026 وحدها) — فعطلات سنةٍ لاحقة غير مذكورة ستقع في التبسيط نفسه (تحذيرٌ
    مبكّر بيوم، لا صمت)، **و`market_calendar.calendar_staleness` تُعلن التقادمَ
    صراحةً قبل نفاده بـ60 يومًا** فلا يمرّ صامتًا."""
    try:
...
def _dump_actor(s, thin=10_000):
    ...
    try:
        srt = None
        # 🔑 خطة 019-ج: أُسقطت رتبةُ `fintel_short` — مفتاحٌ **لا يُكتب في أيّ
        # موضعٍ من المستودع** (رتبةٌ ميتة توهم بمصدرٍ ثالث)؛ ورقمُ Fintel يصل
        # أصلًا عبر `finra_short` (‏:4873 يملؤه من `fintel["short_volume"]` عند
        # غياب FINRA) ⇒ السلسلةُ الآن تسمّي مصادرَها الحقيقية وحدَها.
...
_SPLITS_MEMO: dict = {}     # 🗃️ كاشُ التشغيلة الواحدة (خطة 017) — يُفرَّغ بالعملية
...
def _fetch_splits(sym: str):
    ...
    🗃️ **كاشٌ لكل تشغيلة (خطة 017، 2026-08-15):** الرمزُ الواحد كان يُجلَب 2-3
    مرّات في التشغيلة (تتبّع + تحديث القائمة + مسار المراقبة)، وعلى ~300 سجلًّا
    نشطًا = مئاتُ الرحلات المتسلسلة على **نفس خادم ياهو الذي يخنق
    `download_history`** داخل مهلة الجوب — وهو صنفُ عيب `monitor_pullback`
    المُصلَح سلفًا. التقسيماتُ **ثابتةٌ داخل التشغيلة** فالكاشُ لا يغيّر قيمةً،
    **ويُخزَّن `None` أيضًا** فيبقى سلوكُ العطل حرفيًّا (لا إعادةَ محاولةٍ تُغيّر
    الحكم بين نداءين)."""
    if sym in _SPLITS_MEMO:
        return _SPLITS_MEMO[sym]
    try:
        if yf is None:
            _SPLITS_MEMO[sym] = None
            return None
        out = yf.Ticker(sym).splits
    except Exception:
        out = None
    _SPLITS_MEMO[sym] = out
    return out
...
def entry_status(s: dict) -> dict:
    ...
    try:
        ...
        if mode in ("near_support", "sweep_confirmed"):
            # 🟢🔴 **«لا إشعارَ إلا داخل النطاق»** (قرار المالك 2026-08-06 · الخيار ①):
            #    شرطٌ **ثانٍ** فوق `entry_mode`: السعرُ يجب أن يكون **داخل** نطاق
            #    الدفعات (أو تحته فوق الدعم = تعبئةٌ أفضل) — لا فوق أعلى دفعةٍ بتسامح
            #    الـ5% الذي كان يمرّره `entry_mode`. **والسببُ ميكانيكيّ لا تشدّد:**
            #    الدفعاتُ أوامرُ حدٍّ **تحت** السوق فسعرٌ فوق أعلاها **لا يُعبّئ** ⇒
            #    «ادخل الآن» كانت تصفُ حالةً لا تنفَّذ، والصحيحُ «ضع الطلبات وانتظر».
            #    🔴 والأثرُ المقيس الذي دفع للقرار: توقّعٌ **+0.056R ≈ صفر** مع نجاح
            #    63.2% — لأن الوقفَ يُقاس من الدعم (17.1%) والدخولُ يقع فوق النطاق
            #    فالهدفُ 10.7% ⇒ الرابح **0.65R**.
            # 🔒 **فاشلٌ-آمنٌ مفتوح**: بلا دفعاتٍ أو بلا سعرٍ ⇒ يبقى «جاهز» (المجهولُ
            #    يمرّ بفائدة الشك — كتمُ كرتٍ إزالةٌ، ولا نُزيل على بياناتٍ ناقصة).
            try:
                _tr = [float(x) for x in (s.get("tranches") or []) if x]
                _lp = float(s.get("last_price") or s.get("price") or 0)
                if _tr and _lp > 0:
                    _top = max(_tr) * (1 + float(
                        CONFIG.get("ENTRY_READY_BAND_TOL_PCT", 0.0)) / 100.0)
                    if _lp > _top:
                        _ov = 100.0 * (_lp / max(_tr) - 1.0)
                        return {"status": "watch", "label": "👀 متابعة",
                                "reason": f"فوق نطاق الدفعات بـ{_ov:.1f}% — "
                                          "ضع الطلبات وانتظر النزول للنطاق"}
            except (TypeError, ValueError):
                pass
...
def run_daily_watchlist(wl: dict) -> None:
    ...
    **الشطب بالستوب فقط**؛ الهدف المُحقَّق **لا يُنهي المتابعة** (يبقى نشطًا
    ...
    # 🚪 **فتحُ الباب (2026-08-06، أمرُ المالك «افتح الباب») — امتدادٌ لمنطق ⑨ نفسِه.**
    #    العلّةُ مقيسة: يومَ القرار كان `space = 0` و**‏7 من حاملي الخانات العشرة وسمُهم
    #    `exited`** (‏KLXE·NEGG·TRON·PSTV·LENZ·APVO·QNTM) — أي **خرجوا من النموذج**،
    #    يُحمَلون لمتابعة مركز المالك (قرار 2026-07-21) **ولا يعودون للترشيح أبدًا**
    #    (وبعضُهم يستحيل عودتُه بعد أن صارت M14 صلبة 2026-07-29). فالعرضُ كان **‏3
    #    مؤهَّلين/يوم والمُضاف صفرًا** — انسدادٌ لا تجويع.
    #    ⚖️ والمنطقُ **هو نفسُه** المكتوب في ⑨ أعلاه: مَن انتهى دورُه في **الترشيح**
    #    يبقى نشطًا للمتابعة **ولا يحجز خانةً تمنع مرشّحًا جديدًا**. وصاحبُ `hit` أُعفي
    #    في 2026-07-12 لهذا السبب بعينه، والمحمولُ `exited` أولى منه (ذاك نجح نموذجُه،
    #    وهذا خرج منه).
...
# 🎯 سلّم أهداف ثابتة النسبة — **بديل t2/t3 والتتبّع** بعد أن أسقطتهما المراجعة الذاتية
# (السبب موثَّق في `_exit_policy_r`). النسب من مدى أهداف البوت الفعلي ومن هدف فيصل +100%.
_EXIT_LADDER = (10.0, 20.0, 30.0, 50.0, 100.0)


def _exit_policy_r(t, target_pct=None):
    """R لصفقة واحدة تحت سياسة «اخرج عند هدف نسبته `target_pct`%» (نقيّة).
    المصدر `mg_pre_stop` = أقصى صعود من الدخول **قبل ضرب الوقف**: لو بلغ النسبة ⇒
    خروج عندها ⇒ `R = النسبة ÷ نسبة المخاطرة`؛ وإلا ⇒ ضُرب الوقف أولًا ⇒ **−1R**.

    ⚠️ **حدّان أسقطا صيغتين سابقتين (مراجعة ذاتية 2026-07-27 — قبل الاعتماد):**
    (أ) **t2/t3 غير قابلين للاختبار هنا**: قاموس صفقة الباكتيست يحمل `t1` فقط
    (`backtest_symbol` جذر مقفول لا يُضاف له حقل بلا موافقة المستخدم)، فكانت
    ذراعاهما تُسقَطان **صامتتين** والتقرير يقارن t1 بالتتبّع وحده.
    (ب) **التتبّع غير قابل للتقييم من `mg_pre_stop`**: أي وقف متحرّك يحتاج **مسار
    السعر** لا قمته؛ وصيغة «حصّة من القمة» كانت **متحيّزة تحيّزًا منهجيًّا** — تمنح ربحًا
    مؤكَّدًا لأي قمة موجبة ولو +1% ولا تمثّل خسارة الارتداد إطلاقًا (أعطت 78% «وصول»
    و+0.79R وهي أرقام لا معنى لها). أُسقطت ولم «تُعدَّل».
    فالباقي **وحده الصادق**: أهداف ثابتة النسبة، وهي تجيب السؤال نفسه (هل الطمع أنفع؟)."""
    try:
        e, s0 = float(t["entry"]), float(t["stop"])
        risk = e - s0
        if risk <= 0 or e <= 0 or target_pct is None:
            return None
        ...
        tp = float(target_pct)
        return (tp / risk_pct) if float(mg) >= tp else -1.0
...
def backtest_exit_policies(trades: list) -> list:
    ...
    المُعبَّأة ونفس الوقف**، أيّ **هدف نسبي** يعطي أعلى توقّع R؟ سلّم `_EXIT_LADDER`
    (+10 · +20 · +30 · +50 · +100%) — والأخير هو هدف فيصل الصريح للمقسّم.

    السؤال جاء من قياس 2026-07-27: التوقّع ~0R **لكنه يقيس بيعًا عند t1 حصرًا** وفيصل
    **يمتّع**. ⚠️ حدود مُعلَنة: «بُلغ الهدف» = لمسة داخل الشمعة · الوقف يُفحَص قبل الهدف
    في الشمعة نفسها (فالأرقام **أرضية**) · **لا تُقاس هنا سياسة وقف متحرّك** (تحتاج مسار
    السعر لا قمته — أُسقطت بمراجعة ذاتية بدل تقديم رقم متحيّز). يلزمها `BT_POTENTIAL=1`.
    **تحليل/طباعة فقط — لا تغيّر هدفًا ولا وقفًا؛ الاعتماد يلزمه المعيار + ثلاث سنوات +
    موافقة المستخدم.**"""
    dec = [t for t in trades if t.get("outcome") in ("win", "loss")
           and t.get("mg_pre_stop") is not None]
    if len(dec) < 20:
        return []
    out = ["\n🎯 <b>تجربة T-EXIT: أيّ هدف نسبي يعطي أعلى توقّع؟</b>",
           f"   العيّنة: {len(dec)} صفقة محسومة عندها «أقصى صعود قبل الوقف»."]
    rows = {}
    for tp in _EXIT_LADDER:
        rs = [x for x in (_exit_policy_r(t, tp) for t in dec) if x is not None]
        if len(rs) < 20:
            continue
        ...
        exp = sum(rs) / len(rs)
        rows[tp] = exp
        out.append(f"   هدف +{tp:g}%: توقّع <b>{exp:+.2f}R</b> · "
                   f"بُلغ {hit}/{len(rs)} ({hit / len(rs) * 100:.0f}%، "
                   f"ثقة {lo:.0f}-{hi:.0f}%)")
    if len(rows) < 2:
        out.append("   ⚠️ عيّنة غير كافية للمقارنة — لا حكم هذه السنة.")
        return out
    best_tp = max(rows, key=lambda k: rows[k])
    # المرجع = أقرب نسبة في السلّم لمدى هدف البوت الفعلي (t1 وسطه ~+18%) = +20%
    ref_tp = 20.0 if 20.0 in rows else min(rows)
    gap = rows[best_tp] - rows[ref_tp]
    out.append(f"   📊 الأفضل: <b>هدف +{best_tp:g}%</b> ({rows[best_tp]:+.2f}R) مقابل "
               f"مرجع +{ref_tp:g}% ({rows[ref_tp]:+.2f}R) ⇒ فرق {gap:+.2f}R "
               "(العتبة المسجَّلة 0.15R)")
    if best_tp > ref_tp and gap >= 0.15:
        rec = (f"<b>الطمع أنفع هذه السنة</b> (هدف +{best_tp:g}% يتفوّق) — تلزم "
               "**ثلاث سنوات** بنفس الاتجاه + موافقتك قبل أي تغيير على الأهداف")
    elif best_tp < ref_tp and gap >= 0.15:
        rec = ("<b>الهدف الأقرب أفضل</b> — الطمع يُضعِف التوقّع على هذه السنة "
               "(نتيجة ذات قيمة: تُغلق سؤال «فيصل يمتّع» آليًّا)")
    else:
        rec = ("<b>لا فرق دالّ</b>: مستوى الهدف ليس الرافعة على هذه السنة — "
               "يبقى سلّم الأهداف كما هو")
    out.append(f"   ✅ الحكم بالمعيار المسجَّل مسبقًا: {rec}")
    out.append("   ℹ️ <i>«بُلغ الهدف» = لمسة داخل الشمعة · الوقف يُفحَص قبله في الشمعة "
               "نفسها ⇒ الأرقام أرضية · لا تُقاس هنا سياسة وقف متحرّك (تحتاج مسار "
               "السعر) · t2/t3 غير مُختبَرَين (قاموس الصفقة يحمل t1 فقط والجذر مقفول).</i>")
    return out
...
def record_new_alerts(data, results):
    ...
    for r in results:
        ...
        data["alerts"].append({
            ...
            "short_pct": r.get("short_pct"),
            # 🔒📏 **T-BORROW 🅒 — سدُّ الثقب (2026-08-11، أمرُ المالك «قس المتاح»):**
            # «المتاح للاقتراض» هو **مقياسُ فيصل الظاهر** (‏XHLD «شورته 600 ألف؟ طاخ
            # طيخ» · DSY «7 آلاف متوفّر») — **نجلبه ونعرضه في كلّ كرت ثم نرميه**:
            # فحصُ الجدوى المدوَّن في `borrow_gate_prereg.md` §② وجده **صفرًا** في
            # `alerts_history` (‏0 من 101) و`history` (‏0 من 17)، **وتقاطعُ حصاد
            # الاقتراض بالمحسومة صفرًا** ⇒ **حافّتُه غيرُ قابلةٍ للقياس بأثرٍ رجعيّ
            # لا تاريخيًّا ولا على سجلّنا الحيّ**، وبعد ستّة أسابيع تشغيلٍ لا نملك
            # صفًّا واحدًا يربط «المتاح لحظةَ الترشيح» بنتيجته.
            # ⚖️ **والسابقةُ منصوصةٌ فوق بأربعة أسطر** (‏A3 · 2026-07-03): `tier`/
            # `rsi`/`float` أُضيفت لنفس السبب حرفيًّا — «كلّها متاحة في `r` وقت
            # الإنشاء (**تتبّع فقط — لا يمسّ الفرز**)» ⇒ هذان بنفس الطبقة والمبرّر.
            # 🔒 تسجيلٌ محضّ: صفرُ أثرٍ على الاختيار/الترتيب/الدخول/الوقف/الأهداف/
            #    العضوية · لا `LOGIC_VERSION` · والغائبُ `None` (تعذّر ≠ صفر).
