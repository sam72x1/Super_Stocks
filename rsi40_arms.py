...
# 🔒🔴 **المحورُ مُغلَقٌ بأمر المالك «اقفل RSI 40» (2026-09-09) — والإغلاقُ يُنفَّذ لا يُكتَب.**
# صدر الحكمُ (`rsi40_result.md`): `RS1` ساقطة · والإشارةُ تنقلب (‏−3 · +3 · −5) فالفرقُ
# **ضجيجٌ لا حافّة** · و`R/صفقة` ضدَّ الشدّ في السنوات الأربع. ⇒ لا يُعاد قياسُ هذي
# البوّابة بحسن نيّة. **والحارسُ سلوكيٌّ:** الوالدُ يخرج **6** قبل أن يُطلق عمليةً واحدة.
# 🔓 ولا يُرفَع إلّا بالثلاثة **معًا** (‏`§⑨` من النتيجة، بقرار المالك):
#    ① محورٌ جديدٌ لم يُقَس (‏40 و50 و30 **كلُّها محكومة** ⇒ إعادةُ عتبةٍ ليست محورًا)
#    ② مجتمعٌ يرفع انحيازَ البقاء (كونٌ point-in-time يشمل المشطوبة)
#    ③ تسجيلٌ مسبقٌ جديد ‏+ **إذنُ المالك**
# ⚖️ والمفتاحُ `RSI40_REOPEN` **إقرارٌ لا التفاف**: مَن يضبطه يُعلن أنه استوفى الثلاثة.
AXIS_CLOSED = True
REOPEN_ENV = "RSI40_REOPEN"
CLOSED_RC = 6
_LAST_GATE: list = []
...
def closure_notice() -> list:
    ...
    return [
        "🔒🔴 **محورُ `T-RSI40` مُغلَقٌ بأمر المالك «اقفل RSI 40» (2026-09-09).**",
        "   الحكم: `RS1` ساقطة (‏−3 · +3 · −5) · **الإشارةُ تنقلب فالفرقُ ضجيجٌ لا "
        "حافّة** · و`R/صفقة` ضدَّ الشدّ في السنوات الأربع ⇒ لا ينفع بالمقياسين.",
        "   والتفصيل في `rsi40_result.md` بجانب العقد `rsi40_prereg.md`.",
        "🔓 **ولا يُعاد فتحُه إلّا بالثلاثة معًا:**",
        "   ① محورٌ جديدٌ لم يُقَس — و‏40 و50 و30 **كلُّها محكومة** فإعادةُ عتبةٍ ليست محورًا",
        "   ② مجتمعٌ يرفع انحيازَ البقاء (كونٌ point-in-time يشمل المشطوبة)",
        "   ③ تسجيلٌ مسبقٌ جديد ‏+ **إذنُ المالك**",
...
if __name__ == "__main__":
    ...
    print(f"\n{'=' * 78}\n🏁 رمزُ الخروج: {_rc}"
          + ("  ✅ البوّابةُ عبرت" if _rc == 0
             else "  🔒 **المحورُ مُغلَق** — لا تشغيل" if _rc == CLOSED_RC
