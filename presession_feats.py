...
# 🎚️ **أرضيةُ التسليم — أمرُ المالك «شغّل الأرضية» (2026-09-03).**
# نصُّه قبلها: «‏مب منطقي اني بدخل في 15 سهم … المفروض يوصلني 3 أسهم واهي اللي
# تعتبر **توب التوب**» ⇒ **لا عددٌ ثابتٌ يُرسَل كلَّ ليلة، بل قراءةٌ متطرّفةٌ
# فقط؛ والليلةُ التي لا متطرّفَ فيها تُسلَّم صفرًا.**
#
# 📊 **الرقمُ مقيسٌ لا مخترَع** (`topk_result.md` · العقد `topk_prereg.md` مدفوعٌ
#    قبل أيّ رقم): ذراعُ `F1` **مُعايَرةٌ على 2023-2024 وحدهما** ثم طُبِّقت كما
#    هي على 2025 خارج العيّنة ⟶ **‏47 إصابةً من 435 اسمًا · دقّة 10.805% · رافعة
#    79.7% ⁄ 0.136% = 79.7×** — مقابل «توب 3» بالرتبة **‏46 من 750 بدقّة 6.133%**
#    ⇒ **نفسُ الإصابات بـ‏42% أسماءَ أقلَّ**، و**‏66 ليلةً من 250 صامتة**.
# ⚠️ **والقيمةُ هي المنشورةُ بخمس خاناتٍ معنويّة** كما طُبعت في تشغيلة الحكم
#    `33805138929` — فيُتتبَّع الرقمُ إلى مصدره حرفيًّا، وأثرُ التدوير على قرارٍ
#    واحدٍ دون واحدٍ من مئة ألف.
...
#    **غيرُ صالحةٍ أصلًا**، وهو صامتٌ بقرار المالك على أيّ حال.
# 🗓️🔄 **أُعيدت المعايرةُ بأمر المالك «اعتمد الأرضية المصحّحة» (2026-09-24):
#    ‏0.69492 ⟶ 0.69903.** `T-EARLY` (`early_close_result.md` · التشغيلة
#    `35907136622`) أعاد مسحَ 2023-2025 بإغلاق التقويم فصحّح ثمانيةَ أيّامِ إغلاقٍ
#    مبكّر كانت تُقرأ بحدّ 16:00 ⇒ **طريقةُ المعايرة المسجَّلة نفسُها** (‏2023-2024
#    وحدهما · الخاناتُ الخمس) أعطت على الصفوف الصحيحة **0.69903**. وعلى 2025
#    المصحَّحة خارج العيّنة: **‏47 إصابةً من 429 اسمًا · دقّة 10.956%** (والقديمةُ
#    0.69492 على الصفوف نفسِها 435/47). ⚠️ **ولم يُختَر الرقمُ بأرقام 2025**: هي
#    تُطبَع ولا تحكم — وسببُ الاعتماد أنّ المعايرةَ صارت على بياناتٍ صحيحة.
#    🔒 **والمنشورُ القديم يُعاد كما هو:** `V0` في `presession_report` يُعايِر أرضيتَه
#    من البيانات لا من هذا الثابت · و`tierlink2_probe` يثبّت أرضيةَ عقده 0.69492.
FLOOR_BY_SLOT = {"PM": 0.69903}


def rank_floor(slot: str | None = None) -> float | None:
    """أرضيةُ الجلسة على **مفتاح ترتيبها نفسِه** — أو `None` (لا أرضية)."""
    return FLOOR_BY_SLOT.get(str(slot or "").strip().upper())


def floor_ok(row, slot: str | None = None) -> bool:
    """هل يعبر الصفُّ أرضيةَ جلسته؟ — بالمفتاح نفسِه وباتّجاهه نفسِه.

    🔴 **والمعدومُ يُسقَط لا يمرّ** — خلافًا لعُرف «فاشلٌ-آمنٌ مفتوح»، **وعمدًا**:
    ① لأن هذا **عينُ ما قِيس** (‏`floor_sel` في أداة الحكم تُسقط `nan` بدفعها إلى
    الذيل) فلو مرّ الغائبُ هنا لشُحن غيرُ ما قِيس ② ولأن غيابَ `post_hi_ret` يعني
    حرفيًّا **«لم يكن لأمسِه افترٌ أصلًا»** = نقيضُ الشرط لا نقصٌ في قياسه.
    """
    thr = rank_floor(slot)
    if thr is None:
        return True                       # جلسةٌ بلا أرضية ⇒ السلوكُ السابق بت-بت
    key = rank_key(slot)
    try:
        v = float(row.get(key)) if isinstance(row, dict) else float("nan")
    except (TypeError, ValueError):
        return False
    if not math.isfinite(v):
        return False
    return (v <= thr) if key in FEATS_ASC else (v >= thr)


def apply_floor(rows: list, slot: str | None = None) -> list:
    """يُبقي العابرين **بترتيبهم كما هم** — ولا يُعيد ترتيبًا ولا يُضيف صفًّا.

    🔒 **والأرضيةُ على مفتاح الترتيب نفسِه** ⇒ كلُّ عابرٍ يسبق كلَّ ساقطٍ في
    الترتيب بالبناء ⇒ **الترشيحُ قبل القطع أو بعده يُعطي المجموعةَ نفسَها**
    (مقفولٌ سلوكيًّا) — فلا يُقاس شيءٌ ويُسلَّم غيرُه.
    """
    return [r for r in (rows or []) if floor_ok(r, slot)]
...
def post_feats(post: list, reg_close: float, vol_all: float) -> dict:
    ...
    (‏ويومَ الإغلاق المبكّر 13:00–20:00: الحدُّ من التقويم عند المُنادي · 2026-09-24)."""
