...
# ══════════ 🔒🔴 الإغلاق — **يُنفَّذ لا يُكتَب** (شرطُ العقد §⑨) ══════════
# سقطت `RK2` (وسقط معها الثلاثةُ الباقون) ⇒ **مرجعُ فيصل لا يضيف على العمق
# الخام** ⇒ يُغلَق محورُ «‏RSI مؤشّرَ أولويّةٍ في الفرز». والإغلاقُ **عمليّةٌ لا
# فقرة**: خروجٌ مميَّزٌ **قبل أيّ مدخلٍ أو لقطةٍ أو عمليةٍ ابنة** — لا سطرُ تحذيرٍ
# يُتجاوَز بإعادة تشغيل (سابقتا `T-RSI40` و`T-PMGATE`).
# 🔢 **و‏8 مميَّزٌ عمدًا** عن رموز هذي الأداة الحيّة: ‏2 مدخلٌ · 3 فحصٌ ذاتيّ ·
#    4 طفلٌ/لقطة · 5 حارسٌ ساقط · **6 `V-K2`** · **7 `K27` خامدة** · **9 لا حكم**
#    ⇒ فلا يُقرأ الإغلاقُ خللًا في السجلّ.
# 🔴🔴 **وحارسان لا حارس:** نقطةُ الدخول تتفرّع إلى `run_child` **قبل** `main`،
#    والطفلُ ينادي `run_backtest` وفيها إرسالٌ ووثائقُ CSV بلا حارس ⇒ حارسٌ في
#    `main` وحدَه **يُلتَفّ عليه بـ`--child`**. (وهذي ثغرةٌ قائمةٌ في سابقة
#    `T-RSI40` كُشفت بهذا التدقيق وأُصلحت معها.)
AXIS_CLOSED = True
REOPEN_ENV = "RSIRANK_REOPEN"
CLOSED_RC = 8
...
def closure_notice() -> list:
    ...
    return [
        "🔒🔴 **محورُ «‏RSI مؤشّرَ أولويّةٍ في الفرز» مُغلَق** — بشرط العقد §⑨.",
        "",
        "⚖️ **السبب — بالأرقام لا بالرأي** (`rsi_rank_result.md` · التشغيلة "
        "‏34685861184 · السنوات الثلاث):",
        "   • `RK1` `K27−K0` = ‏+1 · −1 · −1 ⇒ مجمَّع **‏−1** [−6, +4] 🔴",
        "   • `RK2` `K27−C-DEPTH` = ‏+4 · 0 · +3 ⇒ مجمَّع ‏+7 **[−2, +17]** 🔴",
        "   • `RK3` `K27−C-RAND` = ‏+1 · −1 · −1 🔴 · `RK4` `K27−M27` = ‏+1 · −1 "
        "· −1 🔴",
        "   • **والأربعةُ ليست أربعًا:** `C-RAND` و`M27` ≡ `K0` بت-بت في "
        "السنوات الثلاث ⇒ `RK1`/`RK3`/`RK4` **مقارنةٌ واحدة** ⇒ المستقلُّ "
        "**اثنان**.",
        "   • و`P4` المسجَّل («`KR` المقلوبة أسوأُ من `K0` — **وإلّا فالمفتاحُ "
        "صدفة**») **لم يُستوفَ**: ‏36 مقابل 34 مجمَّعًا. ⚠️ وبفارقِ مُسلَّمَين "
        "تحمله سنةٌ واحدة (‏2023 ‏+3 · 2024 ‏−1 · 2025 صفر) وبلا فاصلٍ ⇒ "
        "**تكذيبٌ ضعيفٌ لا قاطع** — ويُقال كما هو.",
        "",
        "🔓 **ولا يُعاد فتحُه إلّا بثلاثةٍ معًا (نصُّ §⑨):**",
        "   ① **مصدرٌ ثانٍ للدعوى غيرُ الدليل التعليميّ والتفريغ الآليّ** — "
        "وإعادةُ عتبةٍ أو نطاقٍ ليست مصدرًا.",
        "   ② **تسجيلٌ مسبقٌ جديد** — والعقدُ الحاليُّ مدموجٌ لا يُعدَّل. ويلزمه "
        "**شاهدٌ حيٌّ لا ينحلّ** (‏`C-RAND`/`M27` انحلّا هنا) و**ميزانيّةٌ "
        "تُخرج الفروقَ من نطاق العددِ المفرد** (‏`d100` الحاكمُ ‏8-14 للسنة).",
        "   ③ **إذنُ المالك.**",
        "",
...
def _closed_now() -> bool:
    """أمُغلَقٌ الآن؟ — يُقرأ المفتاحُ **وقت النداء** لا وقت الاستيراد."""
...
def run_child(year: str) -> int:
    # 🔒🔴 الحارسُ **قبل استيراد الإنتاج**: `run_backtest` فيها `send_telegram`
    #    و`send_telegram_document` و`_write_csv_file` بلا حارس، وتجريدُ مفاتيح
    #    تلغرام يقع في **الوالد** فلا يُصيب استدعاءً مباشرًا بـ`--child`.
    if _closed_now():
