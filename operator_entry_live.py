...
def _maybe_presession(seen, mod, day) -> bool:
    ...
    مقارنةُ البيئة هنا وهناك فتتفرّق. 🗓️ **والمشحونُ `PM,AH` منذ 2026-09-05** بأمر
    «شغّل الافتر» (‏`operator_entry.yml`) — كان هذا السطر يقول `PM` وحدَها فبات
    (‏تصحيحٌ مؤرَّخ 2026-09-24).
    🗓️ **وقرارُ الافتر يتبع إغلاقَ التقويم** (‏2026-09-24، «صلّح الرادار يوم الإغلاق
    المبكر»): يُمرَّر اليومُ إلى `slot_now` فيقع القرارُ 12:50 يومَ الإغلاق المبكّر.
    ...
    # 🔴 **تمييزُ «فشلَ المسحُ» عن «لم يعبر أحدٌ الأرضية»** (‏2026-09-03): بعد
    ...
    elif not rows and diag.get("scanned") and not diag.get("cov"):
        # ⛔ **صفرُ شموعٍ عطلٌ لا صمت** (‏2026-09-24): بهذا الوسم «صامتة» عاش عطلُ بريماركت
        #    الاثنين ثلاثةَ أسابيع (‏09-14 و09-21 `cov=0`) وكأنه ليلةٌ بلا عابر.
        _why = "⚠️ عطلُ جلب — 0 بشموعِ دقيقة (أُرسل تنبيهُ العطل لا «لا أحد»)"
    elif not rows:
...
def main():
    ...
    _log(f"💰 كونُ السيولة (**بلا استثناء**): {len(uni_all)} سهمًا · خيوط "
         ...
         # 🔁 **وإعادةُ المِرساة تُعلَن بسقفها ووسمِها** (أمرُ المالك 2026-09-05):
         #    الوسمُ لا يظهر إلّا يومَ تقع إعادة ⇒ صمتُ السجلّ **لا يفرّق** «لا
         #    إعادةَ اليوم» عن «الوسمُ غيرُ موصول» — وهو عينُ «شرطٌ بلا سطرٍ =
         #    دعوًى غيرُ قابلةٍ للفحص» (نظيرُ `OEL11`). عرضٌ بحت.
    ...
    def _liq_sweep():
        ...
        **مرّتين في الدورة** (إصلاح 2026-08-19، بلاغُ المالك على `$WXM` «برضو
        ...
        # 🔁⚓📜 **وسمُ «مِرساة #N» يُطبَع في السجلّ** (أمرُ المالك 2026-09-05
        #    «طبع الوسم في السجلّ»): يُلتقَط **قبل** الفلتر فيُقرأ المكتومُ
        #    أيضًا — ولولا ذلك لَغاب الوسمُ كلَّما كتم الفلترُ أحداثَ الإعادة
        #    (وقع حيًّا: `M30` و`Px` لـ`GIPR` مكتومان). والنصُّ من
        #    `bot.liq_rearm_tag` **نفسِها** التي يبنيها الكرت ⇒ ما يُطبَع هو
        #    نصُّ الوسم الذي وصل المالك لا وصفٌ مكافئ.
        _lmarks = bot.liq_rearm_marks(lrows) if lrows else []
        _lmuted = []
        if lrows:
            try:
            ...
            if not lrows and _lmarks:   # كُتم الكلّ ⇒ الوسمُ يُطبَع مع ذلك
                _log("🔁 مِرساةٌ مُعادة (مكتومةُ الأحداث): "
                     + " · ".join(_lmarks))
