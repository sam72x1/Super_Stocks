...
# 🔒🔴 **المحورُ مُغلَقٌ بأمر المالك «اقفل البوّابة» (2026-09-11) — والإغلاقُ
#    يُنفَّذ لا يُكتَب** (سابقةُ `T-RSI40`). صدر الحكمُ (`pmgate_result.md`):
#    **الفرعُ 3 — «لا حكم»** · `V-P4` ساقطٌ في الثلاث · **و`C-MOM` (ضبطُ الزخم
#    الصرف، مكتوبٌ في العقد قبل أيّ رقم) يفصل ‏+18.25 نقطةً مقابل ‏+13.74
#    للبوّابة الفيصليّة** ⇒ مرجعُ «قمّة الافتر» **لا يضيف بل ينقص ‏4.5 نقاط**،
#    والساعةُ المذكورة ليست مميَّزةً (‏Δ رتيبٌ صاعدٌ عبر البوّابات الأربع).
# ⇒ رفعُ العيّنة يشتري **دقّةً لنتيجةٍ نعرف اتّجاهَها سلفًا**، فلا يُعاد التشغيل.
# 🔓 ولا يُرفَع الحارسُ إلّا بالثلاثة **معًا** (‏§⑦ من النتيجة، بقرار المالك):
#    ① محورٌ جديدٌ لم يُقَس — و‏04:15 و04:30 و04:45 و05:00 **كلُّها مقيسة**
#      فساعةٌ أخرى ليست محورًا · **أو مصدرٌ ثانٍ للدعوى غيرُ التفريغ الآليّ**
#    ② تسجيلٌ مسبقٌ يجعل **`C-MOM` حاكمًا لا وصفيًّا** ‏+ ميزانيّةٌ تعبر `V-P4`
#      **سنةً سنةً** (‏≈4× عيّنةَ `L`/`Z` في 2023 و≈2.4× في 2024)
#    ③ **إذنُ المالك**
# ⚖️ والمفتاحُ `PMGATE_REOPEN` **إقرارٌ لا التفاف**: مَن يضبطه يُعلن أنه استوفاها.
# 🔢 ورمزُ الخروج **‏8** لا 6: السادسةُ محجوزةٌ لسقوط `V-P5` داخل القياس،
#    ورمزٌ مشترَكٌ يجعل الإغلاقَ **غيرَ مميَّزٍ عن عطب** في السجلّ.
AXIS_CLOSED = True
REOPEN_ENV = "PMGATE_REOPEN"
CLOSED_RC = 8
...
def closure_notice() -> list:
    """نصُّ الإغلاق وشرطُ فتحه — **دالّةٌ نقيّة** ليُقفَل مضمونُها لا شكلُه."""
    return [
        "🔒🔴 **محورُ `T-PMGATE` مُغلَقٌ بأمر المالك «اقفل البوّابة» (2026-09-11).**",
        "   الحكم: **الفرعُ 3 — «لا حكم»** · `V-P4` ساقطٌ في السنوات الثلاث "
        "(‏n_eff `G-YES` = 37.3 · 63.8 · 105.7 والحدُّ 150).",
        "   **و`C-MOM` — ضبطُ الزخم الصرف، مكتوبٌ في العقد قبل أيّ رقم — يفصل "
        "‏+18.25 نقطةً مقابل ‏+13.74 للبوّابة الفيصليّة** ⇒ مرجعُ «قمّة الافتر» "
        "لا يضيف بل ينقص ‏4.5 نقاط · و«‏11 ونص» ليست ساعةً مميَّزة.",
        "   والتفصيل في `pmgate_result.md` بجانب العقد `pmgate_prereg.md`.",
        "🔓 **ولا يُعاد فتحُه إلّا بالثلاثة معًا:**",
        "   ① محورٌ جديدٌ لم يُقَس — و‏04:15 و04:30 و04:45 و05:00 **كلُّها مقيسة** "
        "فساعةٌ أخرى ليست محورًا · أو **مصدرٌ ثانٍ للدعوى** غيرُ التفريغ الآليّ",
        "   ② تسجيلٌ مسبقٌ يجعل **`C-MOM` حاكمًا لا وصفيًّا** ‏+ ميزانيّةٌ تعبر "
        "`V-P4` **سنةً سنةً** (‏≈4× عيّنةَ `L`/`Z` في 2023 و≈2.4× في 2024)",
        "   ③ **إذنُ المالك**",
        f"⚙️ وللتشغيل بعد استيفائها: `{REOPEN_ENV}=1` — وهو **إقرارٌ لا التفاف**.",
    ]


def main() -> int:                                               # noqa: PLR0911
    # 🔒 **الإغلاقُ أوّلًا** — قبل أيّ مدخلٍ وأيّ فحصٍ وأيّ جلبة، فلا يُستهلَك
    #    رنرٌ ولا يُقرأ رقمٌ لمحورٍ صدر حكمُه (‏`PGA12` يُثبته سلوكيًّا).
